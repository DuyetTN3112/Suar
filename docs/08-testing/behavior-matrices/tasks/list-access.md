...
| Tasks | List/access task | Pending member | TASK-LA-SC03 pending member denied | TASK-LA-TC003 | Pending member | Pending membership | Project tasks exist | forbidden state | Canonical Project board request | Load board | No task rows/forbidden | No write | Empty/denied Project board state; no Org task list | None | covered | N/A | N/A | partial | `list_tasks.spec.ts` | partial | partial |
...
| Tasks | List/access task | Legacy route redirect | TASK-LA-SC05 legacy URL enters Project board | TASK-LA-TC005 | Approved project participant | Session has accessible `current_project_id` | Project tasks exist | compatibility URL | `/tasks` or `/org/tasks*` | Open legacy URL | Redirect to `/projects/:projectId/tasks` | No write | Canonical Project Kanban renders; no Org list UI | None | covered | N/A | covered | N/A | `realm_separation_source.spec.ts`, task and Org task redirect controllers | covered | covered |
