...
| TASK-009 | Project manager and superadmin can create task | Project manager / superadmin | Valid create payload | Task persisted | Create flow available | `create_task.spec.ts` | covered backend, missing UI |
| TASK-010 | Task create page renders required controls | Logged-in user | `/tasks/create`, project selection | No backend write required | Title, description, project picker, validation stay on page | `inertia/apps/user/tests/e2e/tasks/task_create_role_prefill.spec.ts` | partial |
...
| TASK-018 | Org admins see org-scoped list, members see own/assigned only | Approved admin/member | List tasks | Query returns role-appropriate rows | `/tasks` should match same visibility | `app/modules/tasks/tests/backend/integration/list_tasks.spec.ts` | covered backend, partial UI |
...
| TASK-020 | List filters project backlog vs active sprint separately | Project tasks in sprint/backlog states | Filter list | Correct filtered rows | Board/list filter should match | `list_tasks.spec.ts` | covered backend, missing E2E |
| TASK-021 | `/org/tasks` can be implicitly filtered by current project | Session has `current_project_id` | Visit org task list without explicit org-wide scope | Runtime may filter by current project | UI can appear to hide org tasks | `docs/09-operations/production-incident-first-response.md` | documented risk, missing test |
...
| TASK-029 | Status board dialog opens, closes, submits, and resists UI crashes | User on status-board page | Dialog interactions | Status API mocked/seeded by test helper | Dialog behavior visible, no console crash | `inertia/apps/user/tests/e2e/tasks/dialog_reactivity_matrix.spec.ts` | partial, not workflow proof |
| TASK-030 | Status board pagination renders | Status board slice | Load component | API data shape accepted | Pagination controls render | `inertia/apps/*/tests/modules/tasks/status_board.test.ts` | partial |
| TASK-031 | Board-state POC conflict path is separate from full persisted board engine | Task board patch conflict scenario | `PATCH /api/tasks/board-state` | Conflict handling behavior, not full engine guarantee | Conflict UI expected | `docs/01-business/features/task_workflow_and_submission.md`, `docs/09-operations/production-incident-first-response.md` | documented risk, partial tests |
...
- List visibility is strong in backend tests but weak at UI/E2E level, especially `/org/tasks` with implicit `current_project_id`.
...
1. Seeded UI create task success path: create task from `/tasks/create`, assert exact title in list/detail, and verify DB/API status id.
...
3. `/org/tasks` current-project regression: set session current project, load org task list with and without explicit org-wide scope, assert no hidden filtering surprise.
4. Board conflict E2E: simulate conflict on `PATCH /api/tasks/board-state`, assert UI rollback/error instead of silent partial move.
