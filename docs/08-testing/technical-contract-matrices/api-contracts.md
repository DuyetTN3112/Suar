...
| Pagination | Cursor windows do not overlap | API-PAGE-SC01 | API-PAGE-TC001 | Pending reviews/org dispute queues | Older/newer cursor windows have no duplicate rows | `pending_reviews_cursor.spec.ts`, `org_dispute_queue_access.spec.ts` | covered | partial | partial | weak | medium |
