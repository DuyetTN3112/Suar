...
test.describe('Talent Directory and Bookmarks', () => {
  ...
  test('talent directory renders heading and search form', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { level: 1, name: /Danh bạ Talent Tổ chức/i })).toBeVisible()
  ...
  test('talent directory shows talent cards or empty state', async ({ page }) => {
    ...
    await expect(
      page.getByRole('link', { name: 'Hồ sơ' }).first().or(page.getByText(/Không tìm thấy talent|Không có talent/i))
    ).toBeVisible()
  })
  ...
  test('bookmarks workspace renders heading', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { level: 1, name: /Talent đã lưu/i })).toBeVisible()
  })
  ...
  test('bookmarks workspace shows bookmarks or empty state', async ({ page }) => {
    ...
    await expect(
      page.getByRole('link', { name: 'Xem hồ sơ' }).first().or(page.getByText(/Chưa có talent đã lưu|Không tìm thấy/i))
    ).toBeVisible()
  })
})
