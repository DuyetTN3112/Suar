...
describe('CreateTaskForm', () => {
  it('splits task setup, skills, and contract into tabs', async () => {
    ...
    expect(screen.getAllByText(/Ngày hết hạn/i).length).toBeGreaterThan(0)
    ...
    expect(screen.queryByText(/Tổ chức hiện tại|Current organization/i)).not.toBeInTheDocument()
    expect(screen.queryByText(/Người thực hiện hiện tại|Current assignee/i)).not.toBeInTheDocument()
    ...
    expect(screen.queryByText(/Ngày hết hạn/i)).not.toBeInTheDocument()
  })
})
