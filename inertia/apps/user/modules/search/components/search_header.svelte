<script lang="ts">
  ...
  function matchLabel(strength: MatchStrength | undefined): string {
    switch (strength) {
      case 'exact':
        return t('search_center.match.exact', {}, 'Exact match')
      case 'strong':
        return t('search_center.match.strong', {}, 'Strong match')
      case 'partial':
        return t('search_center.match.partial', {}, 'Partial match')
      default:
        return t('search_center.match.context', {}, 'Context match')
    }
  }
</script>
...
<div class="rounded-2xl border border-border bg-card p-5 shadow-suar-xs">
  <div class="mb-5 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
    <div class="max-w-3xl">
      <p class="text-[11px] font-black uppercase tracking-[0.22em] text-muted-foreground">
        {t('search_center.eyebrow', {}, 'Search intelligence')}
      </p>
      <h1 class="mt-2 text-4xl font-black tracking-normal text-foreground">
        {t('search_center.title', {}, 'Search Center')}
      </h1>
    ...
    <div class="grid min-w-[260px] grid-cols-2 gap-2 text-sm">
      <div class="rounded-xl border border-border bg-background p-3">
        <div class="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">
          {t('search_center.coverage', {}, 'Coverage')}
        </div>
        <div class="mt-1 text-lg font-black text-foreground">
          {t('search_center.coverage_value', { results: totalResults, domains: nonEmptyDomainCount }, ':results results across :domains domains')}
        </div>
      </div>
      <div class="rounded-xl border border-border bg-background p-3">
        <div class="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">
          {t('search_center.top_signal', {}, 'Top signal')}
        </div>
        <div class="mt-1 truncate text-lg font-black text-foreground">
          {strongestResult ? matchLabel(strongestResult.matchStrength) : t('search_center.no_query', {}, 'No query')}
        </div>
      </div>
    </div>
  </div>
  ...
  <form class="relative" onsubmit={submitSearch}>
    ...
    <input
      ...
      name="q"
      placeholder={t('search_center.placeholder', {}, 'Search tasks, projects, comments, talent, skills, organizations...')}
    />
    <button
      ...
      type="submit"
    >
      {t('search_center.search_button', {}, 'Search')}
    </button>
  </form>
</div>
