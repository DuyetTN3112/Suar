<script lang="ts">
  ...
  import { FRONTEND_ROUTES, getTaskApplyRoute, getTaskDetailRoute } from '@/apps/user/shared/constants'
  ...
  import TaskDetailsSidebar from '@/apps/user/modules/tasks/components/detail/task_details_sidebar.svelte'
  ...
  import SkillRequirementsTab from '@/apps/user/modules/tasks/components/skill_requirements_tab.svelte'
  
  import TaskContextCard from '@/apps/user/modules/tasks/components/detail/task_context_card.svelte'
  ...
  import {
    ...
    statusColors,
    type TaskShowProps,
  } from '@/apps/user/modules/tasks/lib/helpers/show_helpers'
  ...
  const taskDetailUrl = $derived(getTaskDetailRoute(task.id))
        ...
        <Tabs value={activeTab} onValueChange={(value) => { activeTab = value as TaskShowTab }}>
          ...
          {#if canOpenWorkTabs}
            ...
            <TabsContent value="files" class="mt-4">
              <TaskFilesTab taskId={task.id} {currentUserId} apiBase={taskApiBase} />
            </TabsContent>
          {/if}

          {#if canOpenDiscussion}
            <TabsContent value="discussion" class="mt-4">
              <TaskDiscussionTab taskId={task.id} {currentUserId} apiBase={taskApiBase} />
            </TabsContent>
          {/if}
