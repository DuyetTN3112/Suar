<script lang="ts">
  ...
  interface Props {
    ...
    onReportDispute: (reviewMessageId: string, payload: {
      disputeType: string
      claim: string
      evidence: string
  ...
  const canEditResponse = (message: DetailUserMessage) =>
    Boolean(
      message.author_id === currentUserId &&
      ...
      <article class="border-b border-border px-4 py-4 last:border-b-0 sm:px-5">
        ...
        {#if message.message_type === 'review' && message.author_id === currentUserId && canEditSubmittedReview}
          <div class="mt-3 flex flex-wrap items-center gap-2">
            <button type="button" class="inline-flex min-h-8 items-center rounded-md px-2.5 py-1 text-xs font-semibold text-primary transition hover:bg-primary/10" onclick={onBeginEditReview}>
              {t('task.review_workflow.edit_review', {}, 'Edit review')}
            </button>
            <button type="button" class="inline-flex min-h-8 items-center rounded-md px-2.5 py-1 text-xs font-semibold text-destructive transition hover:bg-destructive/10" onclick={() => onWithdrawMessage(message)}>
        ...
        {#if editingReview && message.id === mySubmittedReview?.id}
          <div class="mt-4 space-y-2 border-t border-border pt-3">
            ...
            <div class="flex flex-wrap gap-2">
              <button type="button" class="inline-flex min-h-9 items-center gap-1.5 rounded-md bg-primary px-3 py-1.5 text-xs font-bold text-primary-foreground" onclick={onSubmitReview}><Send class="h-3.5 w-3.5" />{t('task.review_workflow.update_review', {}, 'Update review')}</button>
              <button type="button" class="min-h-9 rounded-md border border-border bg-background px-3 py-1.5 text-xs font-bold" onclick={onCancelEditReview}>{t('common.cancel', {}, 'Cancel')}</button>
            </div>
          </div>
        {/if}
          ...
          <div class="mt-4 border-t border-border pt-3 text-sm">
            ...
            {#if canEditResponse(reply)}
              <div class="mt-2 flex flex-wrap items-center gap-2">
                ...
                <button type="button" class="inline-flex min-h-8 items-center rounded-md px-2.5 py-1 text-xs font-semibold text-destructive hover:bg-destructive/10" onclick={() => onWithdrawMessage(reply)}>{t('task.review_workflow.delete_message', {}, 'Delete')}</button>
              </div>
            {/if}
            {#if editingResponseMessageId === reply.id}
              <div class="mt-3 space-y-2 border-t border-border pt-3">
                ...
                <div class="flex flex-wrap gap-2"><button type="button" class="min-h-8 rounded-md bg-primary px-3 py-1 text-xs font-bold text-primary-foreground" onclick={() => handleUpdateResponse(reply)}>{t('task.review_workflow.update_response', {}, 'Update response')}</button><button type="button" class="min-h-8 rounded-md border border-border px-3 py-1 text-xs font-bold" onclick={cancelEditResponse}>{t('common.cancel', {}, 'Cancel')}</button></div>
              </div>
            {/if}
          </div>
        {/each}
        {#if canRespondTo(message)}
          {#if activeResponseReviewId === message.id}
            <div class="mt-4 space-y-2 border-t border-border pt-3">
              ...
              <button type="button" class="min-h-9 rounded-md bg-primary px-3 py-1.5 text-xs font-bold text-primary-foreground" onclick={() => handleRespondReview(message.id)}>{isDisputeDiscussion(message) ? t('task.review_workflow.send_dispute_discussion', {}, 'Send discussion reply') : t('task.review_workflow.send_response', {}, 'Send response')}</button>
            </div>
          {:else}
        ...
        {#if canOpenDispute(message) || canDecide(message) || canReviewerConfirm(message) || (canReportFor(message) && activeReportReviewId !== message.id)}
          <div class="mt-4 flex flex-wrap items-center gap-2 border-t border-border pt-3">
            {#if canOpenDispute(message)}
              <button type="button" class="inline-flex min-h-9 items-center justify-center gap-1.5 rounded-md border border-destructive/30 bg-destructive/5 px-3 py-1.5 text-xs font-bold text-destructive hover:bg-destructive/10" onclick={() => onOpenDispute(message.id)}>
                <AlertTriangle class="h-3.5 w-3.5" />
                {t('task.review_workflow.open_dispute', {}, 'Open dispute')}
              </button>
            {/if}
            {#if canDecide(message)}
            <button type="button" class="inline-flex min-h-9 items-center justify-center gap-1.5 rounded-md bg-primary px-3 py-1.5 text-xs font-bold text-primary-foreground" onclick={() => onDecideReview(message.id, 'accepted')}>
        ...
        {#if canReportFor(message)}
          {#if activeReportReviewId === message.id}
            <div class="mt-4 space-y-2 border-t border-destructive/30 pt-3">
              ...
              <p class="text-xs text-muted-foreground">{t('task.review_workflow.dispute_report_help', {}, 'Official dispute filing requires claim, evidence, and requested outcome.')}</p>
              <select aria-label="Dispute type" bind:value={disputeType} class="w-full rounded-md border border-input bg-background px-3 py-2 text-sm">
                <option value="deadline">{t('task.review_workflow.dispute_type.deadline', {}, 'Deadline too tight')}</option>
                <option value="scope">{t('task.review_workflow.dispute_type.scope', {}, 'Scope too heavy')}</option>
                <option value="missing_context">{t('task.review_workflow.dispute_type.missing_context', {}, 'Missing context/access')}</option>
                <option value="scope_change">{t('task.review_workflow.dispute_type.scope_change', {}, 'Requirement changed')}</option>
                <option value="review_fairness">{t('task.review_workflow.dispute_type.review_fairness', {}, 'Unfair review')}</option>
                <option value="review_score">{t('task.review_workflow.dispute_type.review_score', {}, 'Incorrect score')}</option>
                <option value="other">{t('common.other', {}, 'Other')}</option>
              </select>
              <textarea id={`task-review-report-${message.id}`} aria-label="Dispute claim" bind:value={disputeClaim} placeholder={t('task.review_workflow.dispute_claim_placeholder', {}, 'What are you disputing?')} class="min-h-20 w-full rounded-md border border-input bg-background px-3 py-2 text-sm"></textarea>
              <textarea aria-label="Dispute evidence" bind:value={disputeEvidence} placeholder={t('task.review_workflow.dispute_evidence_placeholder', {}, 'State basis: task package, review, timeline, or policy.')} class="min-h-20 w-full rounded-md border border-input bg-background px-3 py-2 text-sm"></textarea>
              <select aria-label="Requested outcome" bind:value={requestedOutcome} class="w-full rounded-md border border-input bg-background px-3 py-2 text-sm">
                <option value="task_giver_re_review">{t('task.review_workflow.requested_outcome.task_giver', {}, 'Request task giver re-review')}</option>
                <option value="reviewer_re_review">{t('task.review_workflow.requested_outcome.reviewer', {}, 'Request reviewer re-review')}</option>
                <option value="independent_re_review">{t('task.review_workflow.requested_outcome.independent', {}, 'Request independent reviewer')}</option>
                <option value="adjust_scope_or_deadline">{t('task.review_workflow.requested_outcome.adjust_scope', {}, 'Adjust scope/deadline')}</option>
                <option value="adjust_score">{t('task.review_workflow.requested_outcome.adjust_score', {}, 'Adjust score')}</option>
                <option value="keep_current_review">{t('task.review_workflow.requested_outcome.keep_current', {}, 'Keep current review')}</option>
                <option value="admin_review">{t('task.review_workflow.requested_outcome.admin', {}, 'Request admin review')}</option>
              </select>
              <button type="button" class="min-h-9 rounded-md bg-destructive px-3 py-1.5 text-xs font-bold text-destructive-foreground" onclick={() => handleReportDispute(message.id)}>{t('task.review_workflow.send_report', {}, 'Send report')}</button>
            </div>
          {/if}
        {/if}
      </article>
