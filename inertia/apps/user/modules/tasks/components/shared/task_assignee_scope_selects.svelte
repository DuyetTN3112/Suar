<script lang="ts">
  ...
  import type { TaskVisibilityValue } from '@/apps/user/modules/tasks/lib/rules/task_visibility'
  import {
...
<div class="grid gap-3">
  <div class="grid gap-2">
    <Label for={fieldId}>{fieldLabel ?? t('task.assignee_scope.single_label', {}, 'Người thực hiện (chọn 1 người)')}</Label>
    ...
    <Select value={assignedTo} onValueChange={(value: string) => { onSelect(value) }} disabled={disabled || visibleUsers.length === 0}>
      <SelectTrigger id={fieldId} disabled={disabled || visibleUsers.length === 0} class="min-w-0">
        <span class="min-w-0 flex-1 truncate text-left">
          {#if assignedTo}
            {getUserLabel(visibleUsers.find((user) => user.id === assignedTo) ?? { id: '', username: '', email: '' })}
          {:else}
            {fieldPlaceholder ?? t('task.assignee_scope.single_placeholder', {}, 'Chọn một người thực hiện')}
          {/if}
        </span>
      </SelectTrigger>
