...
import { getDefaultTaskCreateFormData } from './create_task_brief_defaults.js'
import {
  clearTaskCreateDraft,
  persistTaskCreateDraft,
  restoreTaskCreateDraft,
  taskCreateDraftKey,
} from './create_task_draft_storage.js'
import {
  buildTaskFreshPayload,
  cleanFrozenCreatePayload,
...
export function useCreateTaskStore(getProps: () => CreateTaskStoreProps) {
  ...
  let formData = $state<TaskCreateFormData>(getDefaultTaskCreateFormData())
  ...
  const buildPayload = () => {
    if (frozenCreatePayload) {
      frozenCreatePayload = cleanFrozenCreatePayload(frozenCreatePayload)
      return frozenCreatePayload
    }
    const payload = buildTaskFreshPayload(
      formData,
  ...
  function persistStoredDraft() {
    const key = persistTaskCreateDraft(
      formData,
      projectContext.selectedRoleId,
      projectContext.projectProfessionalRoleId
    )
    if (key) {
      activeDraftStorageKey = key
    }
  }

  function restoreStoredDraft(projectId: string, statusId: string) {
    const parsed = restoreTaskCreateDraft(projectId, statusId)
    ...
    projectContext.selectedRoleId = parsed.selectedRoleId ?? ''
    projectContext.projectProfessionalRoleId = parsed.projectProfessionalRoleId ?? ''
    activeDraftStorageKey = taskCreateDraftKey(projectId, statusId)
  }

  function clearStoredDraft(projectId: string, statusId: string) {
    clearTaskCreateDraft(projectId, statusId)
  }
  ...
  const resetForm = () => {
    ...
    frozenCreatePayload = null
    formData = getDefaultTaskCreateFormData()
    errors = {}
    formError = ''
