<script lang="ts">
  ...
  function inputValue(event: unknown): string {
    ...
    if (target && typeof target === 'object' && 'value' in target && typeof (target as { value: unknown }).value === 'string') {
        ...
        <article class={`relative grid gap-4 rounded-xl border bg-muted/20 p-4 pl-12 shadow-sm ${hasError('brief_work_items') ? 'border-destructive/60' : 'border-border'}`}>
          ...
          <div class="grid gap-4 md:grid-cols-2">
            <div class="grid gap-2">
              ...
              <Input id={`work-area-${item.id}`} value={item.affectedArea} placeholder="Ví dụ: chi tiết Task, API lịch sử" oninput={(event) => updateWorkItem(item.id, 'affectedArea', inputValue(event))} />
            </div>
            <div class="grid gap-2">
              <Label for={`work-change-${item.id}`}>Thay đổi phải thực hiện <span class="text-[#ef4444]">*</span></Label>
              <Input id={`work-change-${item.id}`} value={item.requiredChange} placeholder="Ví dụ: kiểm tra quyền trước khi trả dữ liệu" oninput={(event) => updateWorkItem(item.id, 'requiredChange', inputValue(event))} />
            </div>
            <div class="grid gap-2 md:col-span-2">
              <Label for={`work-behaviour-${item.id}`}>Hành vi sau thay đổi <span class="text-[#ef4444]">*</span></Label>
              <Input id={`work-behaviour-${item.id}`} value={item.resultingBehaviour} placeholder="Người dùng hoặc hệ thống có thể quan sát điều gì?" oninput={(event) => updateWorkItem(item.id, 'resultingBehaviour', inputValue(event))} />
            </div>
          </div>
        </article>
      {/each}
    ...
    <div class={`space-y-3 ${hasError('brief_scope') ? 'rounded-xl border border-destructive/60 p-4' : ''}`}>
      ...
      <div class="grid gap-2">
        {#each brief.scope as item (item.id)}
          <div class="flex items-center gap-2"><span class="font-mono text-sm font-bold text-primary">↳</span><Input value={item.text} placeholder="Ví dụ: API đọc lịch sử Task" oninput={(event) => updateLine('scope', item.id, inputValue(event))} /><button class={removeButton} type="button" onclick={() => removeLine('scope', item.id)}>Xóa</button></div>
        {/each}
      </div>
