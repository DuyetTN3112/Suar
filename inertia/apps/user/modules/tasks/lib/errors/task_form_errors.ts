...
function isDirectFormField(path: string): boolean {
  ...
  return root ? DIRECT_FORM_FIELDS.has(root) : false
}

export function normalizeTaskFormErrors(
  error: unknown,
  fallback = 'Unable to process the request. Please try again.'
): NormalizedTaskFormError {
