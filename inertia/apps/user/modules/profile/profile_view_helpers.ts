...
function toEvidenceHistory(value: unknown): SkillEvidenceHistoryEntry[] {
  if (!Array.isArray(value)) return []

  return value
    .filter((item): item is Record<string, unknown> => typeof item === 'object' && item !== null)
    .map((item) => ({
      task_id: toPlainString(item.task_id ?? item.taskId),
      task_title: toPlainString(item.task_title ?? item.taskTitle),
      completed_at: toNullableString(item.completed_at ?? item.completedAt),
      assigned_public_proficiency_code: toNullableString(
        item.assigned_public_proficiency_code ?? item.assignedPublicProficiencyCode
      ),
      reviewer_type: toNullableString(item.reviewer_type ?? item.reviewerType),
      comment: toNullableString(item.comment),
      evidence_links: Array.isArray(item.evidence_links ?? item.evidenceLinks)
        ? ((item.evidence_links ?? item.evidenceLinks) as unknown[])
            .filter((link): link is Record<string, unknown> => typeof link === 'object' && link !== null)
            .map((link) => ({
              evidence_id: toPlainString(link.evidence_id ?? link.evidenceId),
              evidence_type: toPlainString(link.evidence_type ?? link.evidenceType),
              url: toPlainString(link.url),
              title: toNullableString(link.title),
            }))
        : [],
    }))
}
...
export function normalizeProfileSkillRelation(value: unknown): NormalizedProfileSkill {
  ...
  return {
    ...
    verified_public_proficiency_code:
      (relation.verified_public_proficiency_code as string | undefined) ??
      (relation.verifiedPublicProficiencyCode as string | undefined) ??
      null,
    ...
    evidence_count: toNullableNumber(relation.evidence_count ?? relation.evidenceCount) ?? 0,
    evidence_history: toEvidenceHistory(relation.evidence_history ?? relation.evidenceHistory),
    last_reviewed_at:
      toNullableString(relation.last_reviewed_at ?? relation.lastReviewedAt),
    source:
      ...
      null,
    confidence_signal:
      (relation.confidence_signal as 'low' | 'medium' | 'high' | undefined) ??
      (relation.confidenceSignal as 'low' | 'medium' | 'high' | undefined) ??
      null,
    freshness_state:
      (relation.freshness_state as 'unreviewed' | 'fresh' | 'stale' | undefined) ??
      (relation.freshnessState as 'unreviewed' | 'fresh' | 'stale' | undefined) ??
      null,
    governance_state:
      (
        relation.governance_state as 'unreviewed' | 'verified' | 'under_dispute' | undefined
      ) ??
      (
        relation.governanceState as 'unreviewed' | 'verified' | 'under_dispute' | undefined
      ) ??
      null,
  }
}
  ...
  return {
    ...
    items: points.map((point) => {
      ...
      return {
        ...
        verified_public_proficiency_code:
          point.verified_public_proficiency_code ??
          (pointData.verified_public_proficiency_code as string | undefined) ??
          (pointData.verifiedPublicProficiencyCode as string | undefined) ??
          null,
        ...
        freshness_state: totalReviews > 0 ? 'fresh' : 'unreviewed',
        governance_state: totalReviews > 0 ? 'verified' : 'unreviewed',
      }
    }),
  }
}
...
function currentIntlLocale(): string {
  return currentDocumentLocale() === 'vi' ? 'vi-VN' : 'en-US'
}
