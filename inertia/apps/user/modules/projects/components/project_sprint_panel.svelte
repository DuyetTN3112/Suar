<script lang="ts">
  ...
  async function endDelivery(sprintId: string) {
    ...
    if (incompleteTasks.some((task) => !endDestinations[task.id])) {
    ...
    try {
      await axios.post(`/api/v1/projects/${projectId}/sprints/${sprintId}/end-delivery`, {
        incompleteTasks: incompleteTasks.map((task) => {
          ...
          return { taskId: task.id, destination: destination === 'backlog' ? { kind: 'backlog' } : { kind: 'sprint', sprintId: destination } }
        }),
      })
      endingSprintId = null
      await loadSprints()
