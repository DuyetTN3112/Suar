...
<Card class="overflow-hidden">
  <CardHeader class="pb-3">
    <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
      <CardTitle>{t('project.sprint_panel.title_compact', {}, t('project.sprint_panel.title', {}, 'Plan'))}</CardTitle>
  ...
  <CardContent class="space-y-4">
    {#if canManage}
      <div class="grid gap-2 rounded-lg border border-border bg-secondary/40 p-3 md:grid-cols-2 2xl:grid-cols-[1.1fr_1.4fr_1fr_1fr_auto]">
        <input
          class="h-10 w-full min-w-0 rounded-lg border border-border bg-background px-3 text-sm outline-none focus:border-amber-500"
          bind:value={form.name}
          placeholder={t('project.sprint_panel.name_placeholder', {}, 'Sprint name')}
        />
        <textarea
          class="min-h-10 w-full min-w-0 rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none focus:border-amber-500"
          bind:value={form.goal}
          placeholder={t('project.sprint_panel.goal_placeholder', {}, 'Sprint goal')}
          rows="1"
        ></textarea>
        <label class="sr-only" for="project-sprint-start">{t('project.sprint_panel.start_label', {}, 'Sprint start')}</label>
          <input
            id="project-sprint-start"
            class="h-10 w-full min-w-0 rounded-lg border border-border bg-background px-3 text-sm font-normal text-foreground outline-none focus:border-amber-500"
            bind:value={form.startsAt}
            type="datetime-local"
            aria-label={t('project.sprint_panel.start_label', {}, 'Sprint start')}
          />
        <label class="sr-only" for="project-sprint-end">{t('project.sprint_panel.end_label', {}, 'Sprint end')}</label>
          <input
            id="project-sprint-end"
            class="h-10 w-full min-w-0 rounded-lg border border-border bg-background px-3 text-sm font-normal text-foreground outline-none focus:border-amber-500"
            bind:value={form.endsAt}
            type="datetime-local"
            aria-label={t('project.sprint_panel.end_label', {}, 'Sprint end')}
          />
    ...
    {#if sprints.length === 0}
      <div class="rounded-lg border border-dashed border-border bg-background px-4 py-6 text-center text-sm font-semibold text-muted-foreground">
         {t('project.sprint_panel.no_sprints_compact', {}, t('project.sprint_panel.no_sprints', {}, 'No sprints yet.'))}
      </div>
    {:else}
      <div class="grid gap-3">
        {#each sprints as sprint (sprint.id)}
          ...
          <article class="rounded-lg border border-border bg-background p-4 shadow-suar-xs">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <div class="flex flex-wrap items-center gap-2">
                  <h3 class="text-base font-black">{sprint.name}</h3>
                  <span class="rounded-md border border-border bg-secondary px-2 py-0.5 text-xs font-bold text-muted-foreground">
