<script lang="ts">
  ...
  import { buildSignalItems } from './profile_overview_helpers.js'
  import {
    formatCompactNumber,
    formatPercent,
    getUserInitials,
  ...
  import ProfileTaskBreakdownCard from './profile_task_breakdown_card.svelte'
  import ProfileSkillDepthCard from './profile_skill_depth_card.svelte'
  import ProfileCredibilityCard from './profile_credibility_card.svelte'
  import ProfileSignalMetadataGrid from './profile_signal_metadata_grid.svelte'
  import ProfileSummaryStatsGrid from './profile_summary_stats_grid.svelte'
  import type {
...
<section class="relative overflow-hidden rounded-[28px] border border-border bg-card p-5 text-foreground shadow-suar-md">
  ...
  <div class="relative grid gap-5 xl:grid-cols-[minmax(0,1.2fr)_minmax(20rem,0.8fr)]">
    ...
    <div class="space-y-5">
      ...
      <ProfileSignalMetadataGrid {signalItems} />

      <!-- Core summary stats grid -->
      <ProfileSummaryStatsGrid {summaryStats} />
    </div>

    <!-- Right Column: Task breakdown, Skills depth & Credibility -->
    <div class="space-y-4">
      <ProfileTaskBreakdownCard {taskBreakdown} {t} />
      <ProfileSkillDepthCard {skillDepth} {t} />
      <ProfileCredibilityCard
        {displayedCredibilityScore}
        {reviewAccuracy}
        {credibilityMetrics}
        {t}
      />
