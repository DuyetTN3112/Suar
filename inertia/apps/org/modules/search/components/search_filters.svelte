...
<aside class="space-y-3">
  <div class="rounded-2xl border border-border bg-background p-3">
    ...
    <div class="grid gap-2" aria-label="Search result filters">
      {#each tabs as tab}
        <button
  ...
  {#if fieldFacets.length > 0}
    <div class="rounded-2xl border border-border bg-background p-3">
      ...
      <div class="grid gap-2" aria-label="Search match field breakdown">
  ...
  {#if sourceStatuses.length > 0}
    <div class="rounded-2xl border border-border bg-background p-3">
      ...
      <div class="grid gap-2" aria-label="Search source health">
        {#each sourceStatuses as source}
          <div class="rounded-xl border border-border bg-muted/20 px-3 py-2 text-sm">
            ...
            <div class="mt-1 flex items-center justify-between text-xs text-muted-foreground">
              ...
              <span>{source.durationMs ?? 0}ms</span>
            </div>
          </div>
        {/each}
      </div>
  ...
  <div class="rounded-2xl border border-border bg-muted/25 p-4 text-sm leading-6 text-muted-foreground">
    Results are ranked by identity fields first, then description/context/comment matches. Data fanout is capped per source to avoid heavy search bursts.
  </div>
</aside>
