<script lang="ts">
  ...
  const { sprintId, reviewType, targetType, board }: Props = $props()
  ...
  const pageTitle = $derived(
    reviewType === 'environment'
      ...
      : t('task.sprint_reverse_board.title.manager', {}, 'Manager review')
  )
  ...
  function targetName(card: Card): string {
    ...
    return card.target_user?.username
      ...
      ?? (targetId
        ? t('task.sprint_reverse_board.manager_fallback', { id: targetId }, 'Manager :id')
      ...
      <header class="mb-4 flex flex-wrap items-center justify-between gap-3 border-b border-border pb-4">
        ...
        {#if sprintId}
          <div class="rounded-xl border border-border bg-background px-3 py-2 text-sm font-semibold text-muted-foreground">
            {t('task.sprint_reverse_board.sprint_label', {}, 'Sprint')}: {sprintId.slice(0, 8)}
          </div>
        {/if}
      </header>
                  ...
                  {#each section.columns[status].cards as card}
                    ...
                    >
                      <div class="flex items-start justify-between gap-2">
                        <div class="min-w-0">
                          <p class="line-clamp-2 text-sm font-extrabold leading-5 text-foreground">{targetName(card)}</p>
