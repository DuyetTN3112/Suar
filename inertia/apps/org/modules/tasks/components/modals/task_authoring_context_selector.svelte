<script lang="ts">
  ...
  interface AuthoringContextResponse {
    data?: {
      activeProjectContext: { id: string; versionNumber: number; title: string; summary: string; privacyClassification: string } | null
      workPackages: { id: string; key: string; title: string; summary: string; status: 'ready' | 'no_active_version'; activeVersion: { id: string; versionNumber: number; title: string; summary: string; projectContextVersionId: string | null; privacyClassification: string } | null }[]
    }
  }
  ...
  $effect(() => {
    ...
    if (lastProjectId !== projectId) {
      ...
      props.setFormData((previous) => ({ ...previous, project_context_version_id: undefined, work_package_version_id: undefined }))
    }
    context = null
    error = ''
    ...
    fetch(`/api/v1/projects/${projectId}/task-authoring-context`)
      .then(async (response) => { if (!response.ok) throw new Error('Unable to load authoring context'); return (await response.json()) as AuthoringContextResponse })
      .then((payload) => { if (currentRequest === requestKey) context = payload.data ?? null })
      .catch(() => { if (currentRequest === requestKey) error = t('task.create.authoring_context_load_error', {}, 'Không thể tải ngữ cảnh soạn thảo của project.') })
      .finally(() => { if (currentRequest === requestKey) loading = false })
  })

  function selectContext(event: Event) {
    const value = (event.target as HTMLSelectElement).value
    props.setFormData((previous) => ({ ...previous, project_context_version_id: value || undefined }))
  }
  ...
  function selectWorkPackage(event: Event) {
    ...
    props.setFormData((previous) => ({
      ...
      ...(selected?.activeVersion?.projectContextVersionId ? { project_context_version_id: selected.activeVersion.projectContextVersionId } : {}),
    }))
  }
</script>
    ...
    <div class="grid gap-3 md:grid-cols-2">
      <div class="grid gap-2">
        ...
        <select id="project_context_version_id" class="h-10 rounded-md border bg-background px-3 text-sm" value={props.projectContextVersionId ?? ''} onchange={selectContext}>
          ...
          {#if context.activeProjectContext}<option value={context.activeProjectContext.id}>v{context.activeProjectContext.versionNumber} · {context.activeProjectContext.title}</option>{/if}
        </select>
      </div>
      <div class="grid gap-2">
        ...
        <select id="work_package_version_id" class="h-10 rounded-md border bg-background px-3 text-sm" value={props.workPackageVersionId ?? ''} onchange={selectWorkPackage}>
          ...
          {#each context.workPackages as workPackage (workPackage.id)}
            <option value={workPackage.activeVersion?.id ?? ''} disabled={!workPackage.activeVersion}>{workPackage.key} · {workPackage.title}{workPackage.activeVersion ? ` v${workPackage.activeVersion.versionNumber}` : ` · ${t('task.create.no_active_version', {}, 'chưa có phiên bản hiện hành')}`}</option>
          {/each}
        </select>
      </div>
    </div>
