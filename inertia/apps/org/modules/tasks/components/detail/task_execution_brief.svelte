<script lang="ts">
  ...
  import type { TaskResolvedBriefProjection } from '@/apps/org/modules/tasks/types/index.svelte.js'
  ...
  const { task, resolvedBrief = null, onReloadBrief }: Props = $props()
  ...
  <div class="space-y-6 border-t pt-5" data-testid="task-execution-brief">
    ...
    {#if resolvedBrief && hasBriefContent}
      <section class="space-y-4 rounded-xl border border-primary/20 bg-primary/[0.03] p-4" data-testid="resolved-brief">
        ...
        {:else}
          {#if resolvedBrief.resolvedContract.specification?.plainText}
          ...
          {#if hasContractWork}
            <section class="space-y-5" data-testid="task-contract-work">
              ...
              {#if contractDeliverables.length > 0}
                <div>
              ...
              {#if contractAcceptanceCriteria.length > 0}
                <div>
              ...
              <div class="grid gap-4 md:grid-cols-2">
                {#if contractQualityRequirements.length > 0}<div><h6 class="text-sm font-bold">{t('task.execution_brief.quality', {}, 'Yêu cầu chất lượng')}</h6><ul class="mt-2 space-y-2 text-sm">{#each contractQualityRequirements as item}<li class="rounded-lg border bg-background/80 px-3 py-2">{formatContractItem(item)}</li>{/each}</ul></div>{/if}
                {#if contractConstraints.length > 0}<div><h6 class="text-sm font-bold">{t('task.execution_brief.constraints', {}, 'Ràng buộc')}</h6><ul class="mt-2 space-y-2 text-sm">{#each contractConstraints as item}<li class="rounded-lg border bg-background/80 px-3 py-2">{formatContractItem(item)}</li>{/each}</ul></div>{/if}
                {#if contractDependencies.length > 0}<div><h6 class="text-sm font-bold">{t('task.execution_brief.dependencies', {}, 'Phụ thuộc')}</h6><ul class="mt-2 space-y-2 text-sm">{#each contractDependencies as item}<li class="rounded-lg border bg-background/80 px-3 py-2">{formatContractItem(item)}{#if item.state}<span class="ml-1 text-xs text-muted-foreground">({item.state})</span>{/if}</li>{/each}</ul></div>{/if}
              </div>
            </section>
          {/if}
    ...
    {#if task.context_background || task.acceptance_criteria || task.verification_method}
      <section class="space-y-4 rounded-xl border bg-background/80 p-4" data-testid="legacy-task-instructions">
        ...
        {#if task.context_background}<div><h6 class="text-xs font-bold uppercase tracking-wider text-muted-foreground">{t('task.execution_brief.context', {}, 'Bối cảnh')}</h6><p class="mt-2 rounded-lg border bg-muted/20 p-3 text-sm leading-6 whitespace-pre-wrap">{task.context_background}</p></div>{/if}
        {#if task.acceptance_criteria}<div><h6 class="text-xs font-bold uppercase tracking-wider text-muted-foreground">{t('task.execution_brief.acceptance', {}, 'Tiêu chí nghiệm thu')}</h6><p class="mt-2 rounded-lg border bg-muted/20 p-3 text-sm leading-6 whitespace-pre-wrap">{task.acceptance_criteria}</p></div>{/if}
        {#if task.verification_method}<div><h6 class="text-xs font-bold uppercase tracking-wider text-muted-foreground">{t('task.execution_brief.verification', {}, 'Cách tester kiểm tra')}</h6><ul class="mt-2 list-disc space-y-1 pl-5 text-sm">{#each verificationMethods as method}<li>{method}</li>{/each}</ul></div>{/if}
      </section>
    {/if}

    {#if legacyDeliverables.length > 0}
