<script lang="ts">
  ...
  // Evidence can enrich an optional governance report, but it never gates
  // submitting that report or moving the task through its normal workflow.
...
<div class="space-y-5">
  {#if localError}
    <div role="alert" aria-live="assertive" class="rounded-lg border border-destructive/20 bg-destructive/5 px-3 py-2 text-sm text-destructive">
      {localError}
    </div>
  {/if}
  ...
  >
    <div>
      ...
      <p class="mt-1 text-sm text-muted-foreground">
        {t('task.submission_form.readiness_draft', {}, 'Draft can be saved partially after a result summary is provided.')}
      </p>
    </div>
    <div class="space-y-2" aria-labelledby="completion-coverage-heading">
      ...
      <p class="text-xs text-muted-foreground">
        {t('task.submission_form.coverage_not_persisted', {}, 'This guide is not saved as criterion results. Use the report fields and evidence to explain each item.')}
      </p>
    ...
    <div class="grid gap-2 text-sm sm:grid-cols-2" aria-live="polite">
      <p class={summary.trim() ? 'text-foreground' : 'text-muted-foreground'}>
        {summary.trim()
          ...
          : `○ ${t('task.submission_panel.summary_required', {}, 'Please enter a task completion result summary.')}`}
      </p>
      <p class={evidences.length > 0 ? 'text-foreground' : 'text-muted-foreground'}>
        {evidences.length > 0
          ? `✓ ${evidences.length} verification evidence attached`
          : `○ ${t('task.submission_panel.readiness_explanation', {}, 'Evidence is optional governance data. The tester checks the task output against its acceptance criteria.')}`}
      </p>
    </div>
  </section>
