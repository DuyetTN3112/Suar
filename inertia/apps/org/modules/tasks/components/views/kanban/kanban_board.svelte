<script lang="ts">
  ...
  import KanbanColumn from '@/apps/org/modules/tasks/components/views/kanban/kanban_column.svelte'
  ...
  async function handleColumnDrop(event: DragEvent, targetKey: string) {
    ...
    try {
      await onReorderStatuses({
        orderedStatusIds: current,
        previousStatusIds: previous,
      })
    } catch (error) {
      debugKanbanBoard('column reorder failed', { error })
  ...
  function handleRenameStatus(status: string, label: string) {
    onRenameStatus?.({ status, label })
  }
