...
export function isNavUrlActive(currentUrl: string, targetUrl: string): boolean {
  ...
  if (targetQuery) {
    const currentFocus = new URLSearchParams(currentQuery).get('focus')
    const targetParams = new URLSearchParams(targetQuery)
    const targetFocus = targetParams.get('focus')

    if (targetFocus === 'details') {
      return currentFocus === null || currentFocus === 'details'
    }
  ...
  const currentParams = new URLSearchParams(currentQuery)
  return !currentParams.has('focus') && !currentParams.has('projectId')
}

export function resolveBrowserCurrentUrl(currentUrl: string): string {
  ...
  return currentPath === window.location.pathname ? browserUrl : currentUrl
}
