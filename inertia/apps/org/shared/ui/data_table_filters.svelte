<script lang="ts">
  ...
  import DropdownMenuTrigger from '@/apps/org/shared/ui/dropdown_menu_trigger.svelte'
  import DropdownMenuCheckboxItem from '@/apps/org/shared/ui/dropdown_menu_checkbox_item.svelte'
...
<div class="w-full relative group filter-bar-wrapper">
  <div class="flex max-w-full flex-wrap items-center gap-2.5 overflow-hidden p-1.5 bg-black/[0.03] dark:bg-white/[0.03] border border-black/5 dark:border-white/10 rounded-[1.5rem] shadow-[inset_0_1px_1px_rgba(255,255,255,0.05)] transition-all duration-300">
    ...
    {#each filters.filter(f => f.type === 'tabs') as filter}
      <div class="flex w-full min-w-0 items-center overflow-x-auto">
        <Tabs class="w-auto" value={stringValue(values[filter.key]) || stringValue(filter.defaultValue) || 'all'} onValueChange={(value: string) => onFilterChange(filter.key, value)}>
          <TabsList class="h-9 w-max shrink-0 bg-black/5 dark:bg-white/10 rounded-full p-1 border border-black/5 dark:border-white/5">
    ...
    {#each filters.filter(f => f.type === 'date_range') as filter}
      <div class="flex w-full flex-col gap-2 bg-background hover:bg-background/80 border border-border/60 rounded-2xl px-3 py-2 shadow-sm hover:shadow transition-all group/date sm:h-9 sm:w-auto sm:flex-row sm:items-center sm:rounded-full sm:py-0">
        ...
        <Input 
          type="date" 
          class="h-6 w-full min-w-0 text-xs border-none bg-transparent p-0 shadow-none focus-visible:ring-0 text-muted-foreground focus:text-foreground transition-colors sm:w-auto sm:min-w-[110px] sm:max-w-[120px] sm:shrink-0" 
    ...
    {#if children}
      <div class="flex w-full items-center px-1 sm:ml-auto sm:w-auto sm:shrink-0 sm:pl-2">
        {@render children()}
      </div>
    {/if}
