import type { TaskCreateIntent } from '@/apps/shared/tasks/task_create_validation'
...
export const levelNumber = (value: string): number | null => {
...
export function buildTaskCreatePayload(
  ...
  const listItems = (raw: string | undefined) =>
    parseListInput(raw ?? '').map((text) => ({ id: crypto.randomUUID(), title: text, description: text }))
