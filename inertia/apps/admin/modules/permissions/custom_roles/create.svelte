<script lang="ts">
  ...
  async function handleSubmit(e: SubmitEvent) {
    ...
    router.post('/admin/permissions/system/custom-roles', payload, {
      onSuccess: () => {
        toast.success(t('task.admin_permissions.create_success', {}, 'Role created successfully'))
...
<svelte:head>
  <title>{t('task.admin_permissions.create_page_title', {}, 'Admin - Create system role')}</title>
</svelte:head>
...
<div class="space-y-6">
  <div class="flex items-center gap-4">
    ...
    <div>
      <h1 class="text-4xl font-bold tracking-tight">{t('task.admin_permissions.create_title', {}, 'Create new role')}</h1>
      <p class="text-muted-foreground mt-1">{t('task.admin_permissions.create_description', {}, 'Custom roles group system permissions for user assignment.')}</p>
    </div>
  </div>
      ...
      <form onsubmit={handleSubmit} class="space-y-6">
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
          ...
          <div class="space-y-2">
            ...
            <Input 
              ...
              required 
            />
            <p class="text-xs text-muted-foreground">{t('task.admin_permissions.code_hint_create', {}, 'Use lowercase letters, numbers, and underscores only.')}</p>
          </div>
        </div>
        ...
        <div class="flex justify-end gap-3 pt-4 border-t">
          ...
          <Button type="submit" disabled={processing}>
            {processing ? t('task.admin_permissions.processing', {}, 'Processing...') : t('task.admin_permissions.create_submit', {}, 'Create')}
          </Button>
        </div>
      </form>
    </CardContent>
