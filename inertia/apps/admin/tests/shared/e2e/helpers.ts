...
async function bootstrapTestingSessionFromCachedToken(
  ...
  if (requestedOrganizationId && organizationId !== requestedOrganizationId) {
    const refreshResponse = await page.request.post(`${BASE_URL}/api/testing/token-refresh`, {
      form: {
  ...
  const bootstrapResponse = await page.request.post(`${BASE_URL}/api/testing/session/bootstrap`, {
    headers: {
