<script lang="ts">
  ...
  async function renderQrCode(currentVersion: number) {
    ...
    try {
      const svgString: string = await QRCode.toString(qrPayload, {
        type: 'svg',
        width: 320,
        margin: 1,
        color: {
      ...
      if (currentVersion === renderVersion) {
        errorMessage = t('task.admin_subscription_qr.render_error', {}, 'Unable to render QR for this plan.')
        qrCodeUrl = null
      }
    } finally {
      if (currentVersion === renderVersion) {
...
<Card class="overflow-hidden border-primary/20">
    ...
    <div class="flex flex-wrap items-start justify-between gap-3">
      <div class="space-y-1">
        <div class="flex items-center gap-2">
          ...
          <Badge variant="outline">{t('task.admin_subscription_qr.active_subscribers', { count: activeSubscribers }, ':count active')}</Badge>
        </div>
  ...
  <CardContent class="grid gap-6 pt-6 xl:grid-cols-[1.2fr_0.8fr]">
    <div class="space-y-4">
      <div class="grid gap-3 sm:grid-cols-2">
        <div class="rounded-lg border border-border bg-muted/40 p-3">
          <p class="text-xs uppercase tracking-[0.14em] text-muted-foreground">{t('task.admin_subscription_qr.bank', {}, 'Bank')}</p>
      ...
      <div class="space-y-2">
        <Label for={`transfer_ref_${plan.id}`}>{t('task.admin_subscription_qr.transfer_reference_label', {}, 'Reference code / username')}</Label>
        <Input
        ...
        <p class="text-xs text-muted-foreground">
          {t('task.admin_subscription_qr.transfer_content_intro', {}, 'QR will use this transfer content:')}
      ...
      <div class="grid gap-3 sm:grid-cols-2">
        ...
        >
          <div class="flex items-center justify-between gap-2">
            <p class="text-xs uppercase tracking-[0.14em] text-muted-foreground">{t('task.admin_subscription_qr.recipient_account', {}, 'Recipient account')}</p>
        ...
        >
          <div class="flex items-center justify-between gap-2">
            <p class="text-xs uppercase tracking-[0.14em] text-muted-foreground">{t('task.admin_subscription_qr.transfer_content', {}, 'Transfer content')}</p>
          ...
          <p class="mt-1 text-xs text-muted-foreground">{t('task.admin_subscription_qr.personal_upgrade_hint', {}, 'Use for personal Pro / Pro Max upgrades')}</p>
        </button>
      </div>

      <div class="rounded-lg border border-dashed border-border bg-muted/20 p-4">
        <p class="text-xs uppercase tracking-[0.14em] text-muted-foreground">{t('task.admin_subscription_qr.key_benefits', {}, 'Key benefits')}</p>
    ...
    <div class="flex flex-col items-center justify-center rounded-2xl border border-border bg-card p-4 text-card-foreground shadow-xs">
      {#if isLoading}
        <div class="flex min-h-72 flex-col items-center justify-center gap-2 text-muted-foreground">
          ...
          <p class="text-sm">{t('task.admin_subscription_qr.generating_qr', {}, 'Generating QR...')}</p>
        </div>
      ...
      {:else if qrCodeUrl}
        <div class="flex w-full flex-col items-center">
          <img src={qrCodeUrl} alt={t('task.admin_subscription_qr.qr_alt', { plan: plan.name }, 'Payment QR for :plan')} class="h-72 w-72 max-w-full" />
          <Button
            variant="outline"
            class="mt-4 w-full"
            onclick={() => {
          ...
          >
            {#if copied(`payload_${plan.id}`)}
              ...
              {t('task.admin_subscription_qr.payload_copied', {}, 'QR payload copied')}
            {:else}
              <Copy class="mr-2 h-4 w-4" />
              {t('task.admin_subscription_qr.copy_payload', {}, 'Copy QR payload')}
            {/if}
          </Button>
        </div>
      {/if}
