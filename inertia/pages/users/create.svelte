<script lang="ts">
  ...
  const pageTitle = $derived(t('user.create_user', {}, 'Tạo người dùng'))
  ...
  const handleSubmit = () => {
    ...
    if (!formData.password.trim()) {
      newErrors.password = t('user.password', {}, 'Mật khẩu') + ' ' + t('common.is_required', {}, 'là bắt buộc')
    }
    ...
    <Card class="border-2 shadow-neo">
      <CardHeader>
        ...
        <p class="text-sm text-muted-foreground">
          {t('user.create_user_description', {}, 'Thêm người dùng mới bằng cách điền thông tin bên dưới.')}
        </p>
      </CardHeader>
      ...
      <CardContent>
        <div class="grid gap-6">
          ...
          <!-- Password -->
          <div class="grid gap-2">
            <Label for="password" class="font-bold">{t('user.password', {}, 'Mật khẩu')} <span class="text-destructive">*</span></Label>
            <Input
              id="password"
              name="password"
              type="password"
              ...
              onchange={handleChange}
              placeholder={t('user.enter_password', {}, 'Nhập mật khẩu')}
      ...
      <CardFooter class="flex justify-end gap-3 border-t-2 border-border pt-6">
        ...
        <Button
          ...
          disabled={submitting}
        >
          {submitting ? t('common.creating', {}, 'Đang tạo...') : t('user.create_user', {}, 'Tạo người dùng')}
        </Button>
      </CardFooter>
    </Card>
  </div>
