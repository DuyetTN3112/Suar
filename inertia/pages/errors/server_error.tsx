...
function ServerError({ error }: { error: any }) {
  ...
  useEffect(() => {
    // Log lỗi này qua console để debug
    ...
    try {
      ...
      // Log lỗi thủ công qua hệ thống
      if (window.DEBUG?.logViteError) {
        window.DEBUG.logViteError(`Server Error Page: ${error?.message || 'Unknown error'}`);
      }
    } catch (e) {
        ...
        <div style={{ marginTop: '20px', padding: '15px', backgroundColor: 'rgba(0,0,0,0.05)', borderRadius: '4px' }}>
          ...
          <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
            ...
            {isViteError && (
              <button
                onClick={() => {
                  if (window.DEBUG?.scanVitePlugin) {
                    window.DEBUG.scanVitePlugin();
                  }
                }}
                style={{
                  padding: '8px 16px',
