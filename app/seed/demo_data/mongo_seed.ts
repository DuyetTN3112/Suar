...
export async function seedOperationalEvents(
  ...
  await db.table('notifications').insert([
    ...
    },
    {
      ...
      message: 'Bạn đã được approve và tạo external contributor assignment cho task Data Ops.',
      type: 'task_application_approved',
      related_entity_type: 'task',
      related_entity_id: ownerApprovedApplicationTask.id,
      metadata: JSON.stringify({ application_status: 'approved', assignment_type: 'external_contributor' }),
      is_read: false,
    ...
    },
    {
      id: randomUUID(),
      user_id: context.users.owner.id,
      title: 'Đã có người gửi đề xuất tham gia mới cho task marketplace',
      message: 'MaiContributor vừa gửi đề xuất tham gia task public của organization A.',
      type: 'task_application_submitted',
      related_entity_type: 'task',
      related_entity_id: marketplaceTask.id,
      metadata: JSON.stringify({ applicant: context.users.externalContributorOne.username }),
      is_read: false,
      created_at: new Date(runtime.isoDaysAgo(1)),
      updated_at: new Date(runtime.isoDaysAgo(1)),
    },
  ...
  await db.table('audit_events').insert([
    ...
    },
    {
      ...
      new_values: JSON.stringify({ application_status: 'approved', assignment_type: 'external_contributor' }),
      ip_address: '127.0.0.1',
      user_agent: 'seed:data',
      occurred_at: new Date(runtime.isoDaysAgo(1)),
      created_at: new Date(runtime.isoDaysAgo(1)),
