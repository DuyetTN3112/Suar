...
test.group('Review formulas', () => {
  ...
  test('review quorum requires creator review plus one manager and two peer reviews', ({
    assert,
  }) => {
    assert.isFalse(
      isReviewSessionQuorumSatisfied({
  ...
  test('skill weighting, confidence, and composed scores reward stronger and better-supported signals', ({
    ...
    const managerWeightedHigher = calculateSkillWeightedScore([
      { levelCode: 'l12', reviewerType: 'manager', reviewerCredibilityScore: 90, monthsAgo: 1 },
      { levelCode: 'l4', reviewerType: 'peer', reviewerCredibilityScore: 20, monthsAgo: 12 },
    ])
    const peerWeightedLower = calculateSkillWeightedScore([
      { levelCode: 'l12', reviewerType: 'peer', reviewerCredibilityScore: 20, monthsAgo: 12 },
      { levelCode: 'l4', reviewerType: 'manager', reviewerCredibilityScore: 90, monthsAgo: 1 },
    ])

    assert.equal(calculateSkillWeightedScore([]), 0)
    assert.equal(mapLevelCodeToNumber('beginner'), 2)
    assert.equal(mapLevelCodeToNumber('master'), 15)
    assert.equal(mapLevelCodeToNumber('l10'), 11)
    assert.equal(mapWeightedScoreToLevelCode(1.49), 'l0')
    assert.equal(mapWeightedScoreToLevelCode(6.5), 'l6')
    assert.equal(mapWeightedScoreToLevelCode(14.5), 'l14')
