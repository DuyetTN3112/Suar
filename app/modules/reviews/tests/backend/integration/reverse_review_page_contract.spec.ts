...
test.group('Integration | Reverse Review Page Contract', (group) => {
  ...
  test('me scope returns correct shape', async ({ assert }) => {
    ...
    const records = await query.execute({ scope: 'me', page: 1, perPage: 20 })

    assert.isArray(records.data)
    assert.property(records, 'meta')
    assert.property(records, 'stats')
  ...
  test('anonymous reviews hide reviewer_id in org scope', async ({ assert }) => {
    ...
    const query = new ListReverseReviewsQuery(ctx)
    const records = await query.execute({ scope: 'org', page: 1, perPage: 20 })

    const anonymousRecord = records.data.find((r) => r.is_anonymous)
    if (anonymousRecord) {
  ...
  test('org scope masks author identity for anonymous reviews', async ({ assert }) => {
    ...
    assert.isArray(records.data)
    const anonRecords = records.data.filter((r) => r.is_anonymous)
  ...
  test('admin scope returns unmasked data', async ({ assert }) => {
    ...
    const query = new ListReverseReviewsQuery(ctx)
    const records = await query.execute({ scope: 'admin', page: 1, perPage: 20 })
