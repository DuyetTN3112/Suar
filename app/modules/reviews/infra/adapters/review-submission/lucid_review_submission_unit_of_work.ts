...
import { lockClassicReviewSessionGovernance } from '#modules/reviews/infra/adapters/review-core/lucid_classic_review_governance_lock'
...
import { markReviewerAssignmentSubmitted } from '#modules/reviews/infra/adapters/review-session/lucid_review_session_reviewer_assignment_writer'
    ...
    return db.transaction(async (transaction) => {
      const session: ReviewSubmissionPersistenceSession = {
        ...
        loadSessionForUpdate: async (reviewSessionId) => {
          const governance = await lockClassicReviewSessionGovernance(
            transaction,
            reviewSessionId
          )
          ...
          if (
            !row ||
            row.task_assignment_id !== governance.assignmentId ||
            row.reviewee_id !== governance.revieweeId
          ) {
            throw new PersistedDataIntegrityException(
              'Review submission session crossed its locked task-assignment boundary',
              {
                reviewSessionId,
                expectedAssignmentId: governance.assignmentId,
                expectedRevieweeId: governance.revieweeId,
                actualAssignmentId: row?.task_assignment_id ?? null,
                actualRevieweeId: row?.reviewee_id ?? null,
              }
            )
          }
