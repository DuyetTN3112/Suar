  ...
  async loadProjectAccessFacts(
    ...
    const [projectMember, organizationAdministrator] = await Promise.all([
      db
        ...
        // project_members uses (project_id, user_id) as its composite key;
        // it has no synthetic `id` column.
        .select('project_id')
      ...
      db
        ...
        // organization_users uses (organization_id, user_id) as its
        // composite primary key; it has no synthetic id column.
