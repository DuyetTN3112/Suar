import DetectAnomalyCommand from './detect_anomaly_command.js'
...
export default class ProcessReviewSubmittedEventCommand {
  constructor(
  ...
  async handle(event: ReviewSubmittedEvent, context: DeliveryContext = {}): Promise<void> {
    ...
    try {
      await this.transactions.run((transaction) => this.apply(payload, transaction, context.signal))
    } catch (error) {
