...
async function loadSprintReviewDisputeRuntimeContext(
  trx: TransactionClientContract,
  disputeId: string,
  ...
  const reviewerContext = await loadPartyContext(trx, reviewerId, partyScope)
  const counterpartyContext = await loadPartyContext(trx, counterpartyId, partyScope)
...
export default class ReportSprintReviewDisputeCommand {
  ...
  async execute(dto: ReportSprintReviewDisputeDTO): Promise<{ id: string; status: string }> {
    ...
    try {
      ...
      if (dto.escalation_reason.trim().length === 0) {
        throw new BusinessLogicException('Sprint review dispute escalation reason is required')
      }

      const comments = await loadSprintReviewDisputeComments(trx, dto.dispute_id)
      ...
      const runtimeContext = await loadSprintReviewDisputeRuntimeContext(
        trx,
        dto.dispute_id,
        actorId
      )
