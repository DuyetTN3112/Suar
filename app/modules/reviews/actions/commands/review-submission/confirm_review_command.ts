...
> {
  constructor(
    protected override execCtx: ReviewActionContext,
  ...
  async handle(dto: ConfirmReviewDTO): Promise<ReviewConfirmationEntry> {
    ...
    const result = await this.unitOfWork.run(async (persistence) => {
      ...
      const accomplishmentProjection =
        dto.action === 'confirmed' && persistence.resolveAccomplishmentProjectionIdentity
          ? await persistence.resolveAccomplishmentProjectionIdentity(session.id)
          : null
    ...
    await this.settlePostCommitEffect(
      ...
      {
        entityId: dto.review_session_id,
