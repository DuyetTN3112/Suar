...
export default class ProcessReviewConfirmedEventCommand {
  constructor(
    ...
    private readonly accomplishmentProjector?: ReviewConfirmedAccomplishmentProjector
  ) {}
  ...
  async handle(
    ...
    const payload: ReviewConfirmedOutboxPayload = {
      ...
      action: event.action,
      ...(event.accomplishmentProjection
        ? { accomplishmentProjection: event.accomplishmentProjection }
        : {}),
    }
  ...
  private async applyDatabasePhase(
    ...
    if (event.action === 'confirmed' && event.accomplishmentProjection) {
      if (!this.accomplishmentProjector) {
        throw new InvariantViolationException(
          'Review confirmed accomplishment projection capability is not configured'
        )
      }
      await this.accomplishmentProjector.project(event.accomplishmentProjection, trx)
    }
