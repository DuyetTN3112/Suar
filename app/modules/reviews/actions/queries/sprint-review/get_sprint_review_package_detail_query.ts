...
import { resolveEligibleManagerTargets } from '#modules/reviews/domain/sprint-review/sprint_review_rules'
...
export interface GetSprintReviewPackageDetailInput {
  packageId: string
}

export default class GetSprintReviewPackageDetailQuery extends BaseQuery<
  GetSprintReviewPackageDetailInput,
  SprintReviewPackageDetail
> {
  constructor(
  ...
  override executeAndWrap(
    input: GetSprintReviewPackageDetailInput
  ): ReturnType<BaseQuery<GetSprintReviewPackageDetailInput, SprintReviewPackageDetail>['executeAndWrap']>
  override executeAndWrap(
    packageId: string
  ): ReturnType<BaseQuery<GetSprintReviewPackageDetailInput, SprintReviewPackageDetail>['executeAndWrap']>
  override executeAndWrap(
    inputOrPackageId: GetSprintReviewPackageDetailInput | string
  ): ReturnType<BaseQuery<GetSprintReviewPackageDetailInput, SprintReviewPackageDetail>['executeAndWrap']> {
    const input = typeof inputOrPackageId === 'string' ? { packageId: inputOrPackageId } : inputOrPackageId
    return super.executeAndWrap(input)
  }

  async execute(packageId: string): Promise<SprintReviewPackageDetail> {
    return this.handle({ packageId })
  }

  async handle({ packageId }: GetSprintReviewPackageDetailInput): Promise<SprintReviewPackageDetail> {
