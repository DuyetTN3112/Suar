...
const ADMIN_ROLES = new Set<string>([SystemRoleName.SUPERADMIN, SystemRoleName.SYSTEM_ADMIN])
...
export function canOpenReviewDispute(ctx: {
  ...
  const reviewSessionStatus = ctx.reviewSessionStatus as ReviewSessionStatus

  if (ctx.actorId !== ctx.revieweeId) {
    return PR.deny('Only the reviewee can open a review dispute', 'FORBIDDEN')
  }

  if (reviewSessionStatus !== ReviewSessionStatus.COMPLETED) {
...
export function canTransitionDisputeStatus(from: string, to: string): PolicyResult {
  ...
  const terminalStates = new Set<string>(TERMINAL_REVIEW_DISPUTE_STATUSES)
