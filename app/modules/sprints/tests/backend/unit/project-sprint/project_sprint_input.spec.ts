...
test.group('Project sprint input', () => {
  ...
  test('rejects invalid creation status and schedule', ({ assert }) => {
    ...
    const directActiveStatus = captureThrown(() => assertCreateProjectSprintStatus('active'))
    assert.instanceOf(directActiveStatus, ConflictException)
    assert.equal((directActiveStatus as ConflictException).status, 409)
  })
  ...
  test('builds a normalized update while enforcing lifecycle transitions', ({ assert }) => {
    ...
    const activeToReviewConflict = captureThrown(() =>
      buildProjectSprintUpdateAttributes(
        { status: 'review_open' },
        {
          starts_at: '2026-07-16T00:00:00.000Z',
    ...
    assert.instanceOf(activeToReviewConflict, ConflictException)
    assert.equal((activeToReviewConflict as ConflictException).status, 409)
  })
