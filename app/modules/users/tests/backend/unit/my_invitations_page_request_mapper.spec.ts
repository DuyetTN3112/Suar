...
test.group('', () => {
  test('canonicalizes valid pagination aliases to page/perPage', ({ assert }) => {
    assert.deepEqual(
      buildMyInvitationsPageRequest(fakeRequest({ page: '2', per_page: '25' })),
      { page: 2, perPage: 25 }
    )

    assert.deepEqual(
      buildMyInvitationsPageRequest(fakeRequest({ page: '3', limit: '15' })),
      { page: 3, perPage: 15 }
    )

    assert.deepEqual(
      buildMyInvitationsPageRequest(
  ...
  test('rejects invalid page types and values', ({ assert }) => {
    for (const value of [true, {}, 'abc', 0, -1]) {
      assert.throws(
        () => buildMyInvitationsPageRequest(fakeRequest({ page: value })),
        ValidationException
      )
    }
  })
  ...
  test('rejects invalid perPage aliases and values', ({ assert }) => {
    for (const [field, value] of [
      ...
      assert.throws(
        () => buildMyInvitationsPageRequest(fakeRequest({ [field]: value })),
        ValidationException
      )
    }
  })
