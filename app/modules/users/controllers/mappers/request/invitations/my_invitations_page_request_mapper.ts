...
const ORGANIZATION_PAGINATION = definePaginationPolicy()

interface RequestLike {
  input(key: string, defaultValue?: unknown): unknown
}

const MY_INVITATIONS_DEFAULT_PER_PAGE = 10
...
export function buildMyInvitationsPageRequest(
  request: RequestLike
  ...
  const pagination = normalizeStrictPagination(
    ...
    { perPage: MY_INVITATIONS_DEFAULT_PER_PAGE },
    {
      ...
      perPageTooLarge: `perPage must be less than or equal to ${ORGANIZATION_PAGINATION.MAX_PER_PAGE}`,
    }
  )
