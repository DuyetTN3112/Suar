 ...
 */
export default class GetProjectSwitchTargetQuery extends BaseQuery<
  GetProjectSwitchTargetInput,
  ProjectSwitchTarget
> {
  ...
  override async handle(input: GetProjectSwitchTargetInput): Promise<ProjectSwitchTarget> {
    try {
      const project = await this.targets.find(input.projectId, input.userId)
