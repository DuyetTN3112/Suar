 ...
 *
 * TÍCH HỢP VỚI:
 * - Exception classes: #modules/http/exceptions/* (ForbiddenException, NotFoundException, etc.)
 * - Error constants: #modules/errors/public_contracts/error_constants (ErrorCode, ErrorMessages, HttpStatus)
 * - Exception handler: #modules/http/exceptions/handler (tự động xử lý exception theo status code)
 *
 * KHUYẾN NGHỊ:
 ...
 * @example
 * ```typescript
 * import { getErrorMessage, isAppError, AppError } from '#modules/http/errors/error_utils'
 *
 * try {
 *   await someOperation()
 * } catch (error) {
...
/**
 * Legacy note: ưu tiên Exception classes từ `#modules/http/exceptions/index`.
 ...
 *
 * @see {@link file://#modules/http/exceptions/index}
 */
