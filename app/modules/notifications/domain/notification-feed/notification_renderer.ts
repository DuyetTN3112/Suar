...
export function renderNotificationSnapshot(
  ...
  if (command.type === 'task_assigned') {
    ...
    const requesterName = optionalParameter(command, 'requesterName')
    if (requesterName && organizationName) {
      message = `${requesterName} đã gửi yêu cầu tham gia tổ chức "${organizationName}".`
    } else {
    ...
    if (optionalParameter(command, 'reviewKind') === 'sprint_review') {
      ...
      const isSuggested = optionalParameter(command, 'reviewAudience') === 'suggested'
      title = isSuggested ? 'Gợi ý review task' : 'Bạn cần review task'
      const taskTitle = optionalParameter(command, 'taskTitle')
      message = isSuggested
        ? taskTitle
          ? `Bạn có thể review task "${taskTitle}" nếu có thời gian.`
          : 'Bạn được gợi ý review một task có liên quan. Đây không phải yêu cầu bắt buộc.'
        : taskTitle
          ? `Task "${taskTitle}" đang chờ bạn đánh giá.`
          : 'Một task vừa được chuyển vào vùng review và đang chờ bạn đánh giá.'
    }
  } else if (command.type === 'review_received') {
    const reviewEvent = optionalParameter(command, 'reviewEvent')
    if (reviewEvent === 'reviewee_response') {
      title = 'Người thực hiện đã phản hồi review'
      message = 'Hãy mở task để xem phản hồi và tiếp tục xử lý review.'
    } else if (reviewEvent === 'dispute_reply') {
      title = 'Có phản hồi mới trong tranh chấp review'
      message = 'Một bên đã phản hồi trong luồng tranh chấp của task.'
    } else if (reviewEvent === 'reviews_complete') {
      title = 'Review task đã đủ'
      message = 'Các review bắt buộc đã hoàn tất và đang chờ bạn phản hồi.'
    } else if (reviewEvent === 'dispute_resolved') {
      title = 'Tranh chấp review đã được giải quyết'
      message = 'Quản trị viên đã chốt quyết định cho tranh chấp review của task.'
    } else {
      title = 'Bạn vừa nhận được review task'
      message = 'Một người đã gửi review cho task của bạn.'
    }
    ...
    const taskTitle = optionalParameter(command, 'taskTitle') ?? 'nhiệm vụ'
    const reason = optionalParameter(command, 'reason')
    message = optionalParameter(command, 'notificationAudience') === 'reviewer'
      ? `Task bạn đã review đã chuyển trạng thái: ${taskTitle}`
      : `${updaterName} đã cập nhật trạng thái task: ${taskTitle}`
    if (reason) {
      message += ` (${reason})`
    }
