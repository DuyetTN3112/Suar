...
function buildStructuredAuditSummary(
  ...
  const target =
    structured.targetLabel ??
    (structured.targetId ? `#${structured.targetId}` : null)
...
function parseStructuredAuditEvent(
  ...
  const structured: AdminAuditLogStructuredEventSummary = {
    ...
    actorOrganizationId: firstString(
      auditLog.actor_org_id,
      actor?.['organization_id']
    ),
    actorRoleSurface: firstString(
      auditLog.actor_role_surface,
      actor?.['role_surface']
    ),
    targetType: firstString(
      auditLog.target_type,
      target?.['type'],
    ...
    retentionClass: firstString(
      auditLog.retention_class,
    ...
    integrity: {
      ...
      redactionApplied:
        auditLog.redaction_applied === true ||
        compliance?.['redaction_applied'] === true,
...
export function mapAdminAuditLogResponse(auditLog: AdminAuditLogMapperInput) {
  ...
  const structured = parseStructuredAuditEvent(
    safeAuditLog,
    defensiveRedactionApplied,
    auditLog
  )
