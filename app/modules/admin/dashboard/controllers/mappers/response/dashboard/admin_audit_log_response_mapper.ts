...
export interface AuditLogValueMap {
  [key: string]: unknown
}

export interface AdminAuditLogStructuredEventSummary {
...
export interface AdminAuditLogMapperInput {
  id: string
...
function buildStructuredAuditSummary(
  ...
  const target =
    structured.targetLabel ?? (structured.targetId ? `#${structured.targetId}` : null)
...
function buildAuditIntegrityPayload(auditLog: AdminAuditLogMapperInput): Record<string, unknown> {
  return {
    id: auditLog.id,
    user_id:
      auditLog.source_user_id ?? auditLog.actor_user_id ?? auditLog.user?.id ?? null,
    ...
    actor_user_id:
      auditLog.actor_user_id ?? auditLog.source_user_id ?? auditLog.user?.id ?? null,
