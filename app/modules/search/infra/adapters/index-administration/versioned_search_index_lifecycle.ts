...
export interface SearchIndexCandidateResult {
...
export class VersionedSearchIndexLifecycle {
  ...
  async rebuildIndex(
    ...
    const candidate = await this.buildCandidate(populate, generation)
    await this.activateGeneration(candidate.physicalIndexName, candidate.previousIndexNames)
    return {
      activatedIndexName: candidate.physicalIndexName,
      previousIndexNames: candidate.previousIndexNames,
  ...
  async buildCandidate(
    populate: (physicalIndexName: string) => Promise<number>,
    generation = this.buildGeneration()
  ): Promise<SearchIndexCandidateResult> {
  ...
  async reconcileAliasToGeneration(
    physicalIndexName: string,
    expectedCurrentIndexNames: readonly string[]
  ): Promise<void> {
    await this.activateGeneration(physicalIndexName, [...expectedCurrentIndexNames])
  }
