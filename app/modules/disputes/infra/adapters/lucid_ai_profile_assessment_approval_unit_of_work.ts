        ...
        createOrLoadCapabilityApproval: async (
          ...
          const [row] = (await transaction
            ...
            .insert({
              ...
              source_payload_hash: input.sourcePayloadHash ?? payloadHash(input.sourcePayload),
          ...
          const existing = row ?? ((await transaction
            ...
            .first()) as Record<string, unknown> | undefined)
          if (!existing) throw new PersistedDataIntegrityException('Không thể lưu phê duyệt đề xuất hồ sơ')
