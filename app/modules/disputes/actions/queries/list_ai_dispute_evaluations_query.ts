...
} from '#modules/disputes/actions/commands/start_ai_dispute_evaluation_command'
import type {
  AiDisputeEvaluationRecordSource,
  AiDisputeEvaluationSourceReader,
} from '#modules/disputes/actions/ports/outbound/ai_dispute_evaluation_source_reader'
...
function normalize(row: AiDisputeEvaluationRow): AiDisputeEvaluationResult {
  ...
  const profileApprovals = Array.isArray(row.profile_approvals)
    ? row.profile_approvals.filter(
        (entry): entry is { id: string; proposal_index: number; approved_observed_level: string; approved_at: string } =>
  ...
  return {
    ...
    ...(profileApprovals.length > 0 ? { profile_approvals: profileApprovals } : {}),
    request_payload:
...
export default class ListAiDisputeEvaluationsQuery extends BaseQuery<
  ListAiDisputeEvaluationsDTO,
  AiDisputeEvaluationResult[]
> {
  constructor(
  ...
  async handle(dto: ListAiDisputeEvaluationsDTO): Promise<AiDisputeEvaluationResult[]> {
