...
export default class CloneProfessionalRoleTemplateCommand extends BaseCommand<
  CloneProfessionalRoleTemplateInput,
  ProjectProfessionalRoleRecord
> {
  constructor(
  ...
  private async getOrCreateProjectSkillId(
    ...
    const projectSkill = await this.addProjectSkill.execute(
      {
        projectId,
        skillId,
        // Vai trò chỉ phục vụ lọc thành viên. Sau khi sao chép, quản lý
        // Project vẫn phải chọn riêng khoảng Task trước khi kỹ năng này dùng
        // được ở form tạo Task.
        allowUnconfiguredTaskRequirementRange: true,
      },
      transaction
    )
