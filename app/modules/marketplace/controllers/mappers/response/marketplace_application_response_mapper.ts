...
interface MarketplaceApplicationControllerResult {
...
function readStringArray(record: SerializedModelRecord, key: string): string[] {
  const value = readValue(record, key)
...
function mapMarketplaceTaskApplicationsListItem(
  ...
  return stripUndefined({
    ...
    portfolio_links: readStringArray(serialized, 'portfolio_links'),
...
function mapMyMarketplaceApplicationListItem(
  application: SerializableModelRecord | SerializedModelRecord
): SerializedModelRecord {
  const serialized = serializeModelForHttpResponse(application)
...
export function mapMarketplaceApplicationMatchScoreApiBody(result: SerializedModelRecord) {
  return {
...
export function mapMarketplaceTaskApplicationsPageProps(
  result: MarketplaceApplicationControllerResult,
  taskId: string,
  ...
  return {
    ...
    applications: result.data.map((application) =>
      mapMarketplaceTaskApplicationsListItem(application)
    ),
...
export function mapMyMarketplaceApplicationsPageProps(
  ...
  return {
    applications: result.data.map((application) =>
      mapMyMarketplaceApplicationListItem(application)
    ),
    pagination: toCanonicalPagePagination(fromLegacySnakePagination(result.meta)),
    statusFilter: statusFilter ?? 'all',
  }
