...
/**
 * Rejects unsafe production defaults before the application accepts traffic.
 */
export function assertRedisProductionPolicy(input: RedisProductionPolicyInput): void {
  ...
  if (!input.main.password?.trim()) {
    violations.push('REDIS_PASSWORD is required in production')
  }
  if (!input.cache.password?.trim()) {
    violations.push('REDIS_CACHE_PASSWORD is required in production')
  }
  ...
  if (!input.main.tlsEnabled) {
    violations.push('REDIS_TLS_ENABLED must be true in production')
  }
  if (!input.cache.tlsEnabled) {
    violations.push('REDIS_CACHE_TLS_ENABLED must be true in production')
  }
  if (!areRedisEndpointsDistinct(input.main, input.cache)) {
    violations.push(
      'main and cache Redis must use different production endpoints; logical DBs are not isolation'
    )
  }
  if (input.sessionStore !== 'redis') {
    violations.push('SESSION_DRIVER must be redis in production')
  }
  if (input.lockStore !== 'redis') {
    violations.push('LOCK_STORE must be redis in production')
  }
  if (input.limiterStore !== 'redis') {
    violations.push('LIMITER_STORE must be redis in production')
  }
  if (
    !Number.isSafeInteger(input.connectTimeoutMs) ||
    input.connectTimeoutMs < 100 ||
    input.connectTimeoutMs > 30_000
  ) {
    violations.push('REDIS_CONNECT_TIMEOUT_MS must be between 100 and 30000')
  }
  if (
    !Number.isSafeInteger(input.commandTimeoutMs) ||
    input.commandTimeoutMs < 100 ||
    input.commandTimeoutMs > 30_000
  ) {
    violations.push('REDIS_COMMAND_TIMEOUT_MS must be between 100 and 30000')
  }
  if (
    !Number.isSafeInteger(input.maxRetriesPerRequest) ||
    input.maxRetriesPerRequest < 0 ||
    input.maxRetriesPerRequest > 5
  ) {
