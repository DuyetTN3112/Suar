...
test.group('AuthorizeCacheInvalidationOperatorQuery', () => {
  ...
  test('fails closed for missing, inactive, or unauthorized principals', async ({ assert }) => {
    assert.isNull(await serviceFor({ missing: true }).execute('missing'))
    assert.isNull(await serviceFor({ status: 'suspended' }).execute('suspended'))
    assert.isNull(await serviceFor({ allowed: false }).execute('unauthorized'))
  })
})
