...
/**
 * Shared application collaborator that translates role facts into the
...
export async function hasProjectApplicationReviewRole(
  ...
  const role = await permissionReader.getProjectRoleName(userId, projectId, transaction)
...
export async function hasOrganizationApplicationReviewRole(
  ...
  const role = await permissionReader.getOrgRoleName(userId, organizationId, transaction)
