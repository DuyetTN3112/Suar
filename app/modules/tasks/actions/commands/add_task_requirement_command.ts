...
export default class AddTaskRequirementCommand {
  ...
  async execute(input: AddTaskRequirementInput): Promise<TaskRequirementRecord> {
    return this.transactions.run(async (transaction) => {
      ...
      const requiredPublicProficiencyCode =
        await this.resolveCanonicalRequiredLevelCode(
          omitUndefined({
      ...
      return this.requirementWriter.create(
        {
          ...
          source_project_professional_role_id:
            input.sourceProjectProfessionalRoleId ?? null,
  ...
  private async resolveCanonicalRequiredLevelCode(input: {
    ...
    const levelId =
      input.minimumLevelId ?? input.targetLevelId ?? input.assessmentCeilingLevelId ?? null
    if (!levelId) {
      return 'l4'
    }
