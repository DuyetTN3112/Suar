...
export default class RemoveTaskRequirementCommand {
  ...
  async execute(requirementId: string): Promise<void> {
    await this.transactions.run(async (transaction) => {
      ...
      const taskRequirements = await this.requirements.findByTask(
        requirement.task_id,
        transaction
      )
      const remaining = taskRequirements.filter(
      ...
      const referenceFacts =
        await this.skills.findTaskRequirementReferenceFacts({
          skillIds,
          proficiencyLevelIds: [],
        })
      ...
      const missingSkillIds = skillIds.filter(
        (skillId) => !categoryBySkillId.has(skillId)
      )
