...
export default class RemoveTaskRequirementCommand {
  ...
  async execute(requirementId: string): Promise<void> {
    await this.transactions.run(async (transaction) => {
      ...
      if (!requirement) throw new NotFoundException('Task skill requirement not found')
      ...
      const referenceFacts = await this.skills.findTaskRequirementReferenceFacts({ skillIds, proficiencyLevelIds: [] })
      const categoryBySkillId = new Map(referenceFacts.skills.map((skill) => [skill.id, skill.categoryCode]))
      const missingSkillIds = skillIds.filter((skillId) => !categoryBySkillId.has(skillId))
      if (missingSkillIds.length > 0) throw new ValidationException(`Task requirements reference missing skills: ${missingSkillIds.join(', ')}`)
      const violations = getTaskRequiredSkillCategoryViolations(
        countTaskRequiredSkillCategories(remaining.map((candidate) => categoryBySkillId.get(candidate.skill_id)))
      )
      if (violations.length > 0) throw new ValidationException(formatTaskRequiredSkillCategoryViolations(violations))
