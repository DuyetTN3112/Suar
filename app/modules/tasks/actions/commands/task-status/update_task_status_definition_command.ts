...
import { canEditStatus } from '#modules/tasks/domain/task-status/task_status_rules'
import { DOCUMENTATION_TASK_STATUS_SLUG } from '#modules/tasks/public_contracts/task_constants'
 ...
 */
export default class UpdateTaskStatusDefinitionCommand extends BaseCommand<
  UpdateTaskStatusDTO,
  TaskStatusRecord
> {
  constructor(
  ...
  async handle(dto: UpdateTaskStatusDTO): Promise<TaskStatusRecord> {
  ...
  async execute(dto: UpdateTaskStatusDTO): Promise<TaskStatusRecord> {
    ...
    const updatedStatus =
      ...
      if (
        ...
        dto.slug !== undefined &&
        dto.slug !== DOCUMENTATION_TASK_STATUS_SLUG
      ) {
        throw new ConflictException('Không thể đổi mã trạng thái Docs')
      }
