...
import { validateBatchStatusUpdate } from '#modules/tasks/domain/task-assignment/task_assignment_rules'
...
interface BatchUpdateTaskStatusInput {
  taskIds: string[]
...
export default class BatchUpdateTaskStatusCommand {
  ...
  async executeAndWrap(
    input: BatchUpdateTaskStatusInput
  ): Promise<Result<{ updated: number; failed: string[] }, AppException>> {
    try {
      return Result.ok(
        await this.execute(input.taskIds, input.newTaskStatusId, input.organizationId)
      )
    } catch (error) {
    ...
    try {
      persisted = await this.taskExternalDependencies.transactions.run(async (trx) => {
        ...
        for (const task of tasks) {
          ...
          if (!currentStatus) {
            throw new ConflictException(
              `Không thể cập nhật task ${task.id} vì trạng thái hiện tại không còn hợp lệ`
            )
          }

          if (currentStatusId !== newTaskStatusId && newStatus.category === 'done' && !task.assigned_to) {
            throw new BusinessLogicException(
              `Không thể chuyển Task ${task.id} sang Done khi chưa có người thực hiện`
            )
          }
