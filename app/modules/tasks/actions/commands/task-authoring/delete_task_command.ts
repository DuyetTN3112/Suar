...
import { canDeleteTask, canPermanentDeleteTask } from '#modules/tasks/domain/task-assignment/task_permission_policy'
...
> {
   ...
   */
  async handle(dto: DeleteTaskDTO): Promise<{ success: true; message: string }> {
    ...
    const deletionResult =
      ...
      if (this.taskExternalDependencies.searchProjectionInvalidation) {
        await this.taskExternalDependencies.searchProjectionInvalidation.stage({
          taskId: task.id,
          operation: 'delete',
          sourceRevision: occurredAt,
          changedFields: ['deleted_at', 'is_deleted', 'marketplace_visible', 'member_visible'],
        }, trx)
      }
  ...
  execute(dto: DeleteTaskDTO): Promise<{ success: true; message: string }> {
