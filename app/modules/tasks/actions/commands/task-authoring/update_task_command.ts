...
> {
  ...
  async handle(input: UpdateTaskCommandInput): Promise<TaskDetailRecord> {
    ...
    let updateResult
    try {
      updateResult = await this.executeInTransaction(async (trx) => {
        ...
        const sourceRevision = result.task.updated_at ?? result.task.created_at
    ...
    const detail = await this.taskExternalDependencies.lifecycle.findTaskDetail(updateResult.task.id)
    return updateResult.task.authoring
      ? { ...detail, authoring: updateResult.task.authoring }
      : detail
  }
