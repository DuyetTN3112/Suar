...
import { mapTaskListOutput, type TaskListQueryRecord } from '../../mappers/task-reading/task_query_output_mapper.js'
...
export default class GetTasksListQuery {
  ...
  constructor(
    ...
    this.deps = {
      ...
      resolvePermissionFilter: (userId, organizationId, projectId) =>
        this.buildPermissionFilter(userId, organizationId, projectId),
  ...
  async execute(dto: GetTasksListDTO): Promise<TaskListResult> {
    ...
    const permissionFilter = await this.deps.resolvePermissionFilter(
      userId,
      dto.organization_id,
      dto.project_id
    )
  ...
  private async buildPermissionFilter(
    ...
    const [accessContext, actorProjectRole] = await Promise.all([
      ...
      projectId
        ? this.taskExternalDependencies.permission.getProjectRoleName(userId, projectId)
        : Promise.resolve(null),
    ])
    return buildTaskPermissionFilter(
      projectId
        ? {
            ...accessContext,
            actorProjectRole: actorProjectRole === 'unknown' ? null : actorProjectRole,
            projectId,
          }
        : accessContext
    )
