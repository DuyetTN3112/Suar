...
export default class GetUserTasksQuery extends BaseQuery<GetUserTasksInput, UserTaskListResult> {
   ...
   * Execute query
   */
  async execute(options: GetUserTasksInput): Promise<UserTaskListResult> {
    return this.handle(options)
  }

  async handle(options: GetUserTasksInput): Promise<UserTaskListResult> {
    const {
      userId,
      organizationId,
      filterType = 'both',
   ...
   * Build cache key
   */
  private buildCacheKey(options: GetUserTasksInput): string {
    const parts = [
      'task:user',
      `user:${options.userId}`,
