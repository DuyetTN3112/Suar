...
export default class GetTaskMetadataQuery {
  ...
  async execute(organizationId?: string, selectedProjectId?: string | null): Promise<{
    ...
    const loadFromSource = async () => {
      const statuses = await this.loadStatuses(orgId, selectedProjectId ?? null)
      ...
      const proficiencyLevels = await this.taskExternalDependencies.skill.listActiveProficiencyLevels()
   ...
   */
  private async loadStatuses(organizationId: string, projectId: string | null): Promise<
    {
      id: string
      value: string
      label: string
  ...
  > {
    // Task columns are project-owned. Never fall back to the organization
    // template while a project is selected, otherwise a sibling project's
    // workflow can leak into this board.
    const statuses = projectId
      ? await this.taskStatusRepository.findByProject?.(projectId, organizationId) ?? []
      : await this.taskStatusRepository.findByOrganization(organizationId)
  ...
  > {
    if (selectedProjectId) {
      ...
      return projectSkills
        ...
        .map((projectSkill) => ({
          ...
          minimumTaskRequirementLevelId: projectSkill.minimumTaskRequirementLevelId,
          maximumTaskRequirementLevelId: projectSkill.maximumTaskRequirementLevelId,
          minimumTaskRequirementLevelCode: projectSkill.minimumTaskRequirementLevelCode,
          maximumTaskRequirementLevelCode: projectSkill.maximumTaskRequirementLevelCode,
        }))
    }
    ...
    return skills.map((skill) => ({
      ...
      minimumTaskRequirementLevelCode: null,
      maximumTaskRequirementLevelCode: null,
    }))
  }
}
