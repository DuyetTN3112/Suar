...
async function resolveCanonicalRequiredLevelCode(
  options: {
    requiredPublicProficiencyCode?: string | null
  ...
  if (
    options.requiredPublicProficiencyCode &&
    options.requiredPublicProficiencyCode.trim().length > 0
  ) {
    if (!isCanonicalProficiencyLevelCode(options.requiredPublicProficiencyCode)) {
      throw new Error(
        `requiredPublicProficiencyCode must be a canonical code (l0-l14): ${options.requiredPublicProficiencyCode}`
      )
    }
    return getCanonicalProficiencyLevelValue(options.requiredPublicProficiencyCode)
  }
  ...
  const level = await skillPublicApi.findProficiencyLevelById(levelId)
  return getCanonicalProficiencyLevelValue(level?.code, 'l4')
}

async function assertTaskRequirementCategoryMixForTask(
  taskId: string,
  options: {
    excludingRequirementId?: string
  } = {}
): Promise<void> {
  const requirements = await TaskRequirementRepository.findByTaskWithRelations(taskId)
  const remainingRequirements = options.excludingRequirementId
    ? requirements.filter((requirement) => requirement.id !== options.excludingRequirementId)
    : requirements

  const categoryCounts = countTaskRequiredSkillCategories(
    remainingRequirements.map((requirement) => requirement.skill.category_code)
  )
  const categoryViolations = getTaskRequiredSkillCategoryViolations(categoryCounts)

  if (categoryViolations.length > 0) {
    throw new Error(formatTaskRequiredSkillCategoryViolations(categoryViolations))
  }
}
...
export const TaskSkillRequirementService = {
  ...
  async addRequirement(
    ...
    const requiredPublicProficiencyCode = await resolveCanonicalRequiredLevelCode(omitUndefined({
      requiredPublicProficiencyCode: payload.requiredPublicProficiencyCode,
  ...
  async prefillFromProjectRole(
    ...
    const role = await skillPublicApi.findProjectProfessionalRoleById(
    ...
    for (const roleSkill of role.role_skills) {
      ...
      const requiredPublicProficiencyCode = await resolveCanonicalRequiredLevelCode({
        minimumLevelId,
  ...
  async removeRequirement(requirementId: string): Promise<void> {
    ...
    await assertTaskRequirementCategoryMixForTask(requirement.task_id, {
      excludingRequirementId: requirement.id,
    })
