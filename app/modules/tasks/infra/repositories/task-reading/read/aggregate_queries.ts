...
export const countIncompleteByProject = async (
  ...
  const result = await makeTaskReadQuery(trx)
  ...
  return toNumberValue(readTaskModelExtraField(result[0], 'total'))
}
...
export const getTasksSummaryByProject = async (
  ...
  for (const task of tasks) {
    const statusCategoryValue = readTaskModelExtraField(task, 'status_category')
...
export const countByAssignees = async (
  ...
  let query = makeTaskReadQuery(trx)
    .where('tasks.project_id', projectId)
    .whereNull('tasks.deleted_at')
  ...
  for (const row of results) {
    if (row.assigned_to) {
      map.set(row.assigned_to, toNumberValue(readTaskModelExtraField(row, 'total')))
    }
  }
  return map
}
...
export const countByProjectIds = async (
  ...
  const results = await makeTaskReadQuery(trx)
  ...
  for (const row of results) {
    if (row.project_id) {
      map.set(row.project_id, toNumberValue(readTaskModelExtraField(row, 'total')))
    }
  }
  return map
}
...
export const countByTaskStatusId = async (
  ...
  const row = await makeTaskReadQuery(trx)
    .where('tasks.task_status_id', taskStatusId)
    .whereNull('tasks.deleted_at')
    .count('* as total')
    .first()

  return toNumberValue(readTaskModelExtraField(row, 'total'))
}
