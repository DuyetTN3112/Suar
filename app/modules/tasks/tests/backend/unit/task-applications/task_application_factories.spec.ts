...
import ListTasksController from '#modules/tasks/controllers/task-reading/list_tasks_controller'
...
test.group('Task application factories', () => {
  ...
  test('application provider resolves all controller-facing capabilities', async ({ assert }) => {
    const [
      ...
      app.container.make(ListTasksController),
    ...
    assert.instanceOf(myWorkController, ListTasksController)
