...
test.group('Task assignment rules', () => {
  ...
  test('canProcessApplication allows task creator, project reviewer, and org leader, blocks approving already assigned tasks', ({
    assert,
  }) => {
    ...
    // Organization owner/admin can process even if not creator or project reviewer
    assert.isTrue(
      canProcessApplication({
        actorId: 'user-003',
        taskCreatorId: 'user-001',
    ...
    // Non-creator non-manager non-org-leader denied
    assert.isFalse(
      canProcessApplication({
        actorId: 'user-002',
        taskCreatorId: 'user-001',
    ...
    // Organization owner/admin cannot approve if already assigned
    assertDenied(
      assert,
      canProcessApplication({
        actorId: 'user-003',
