...
test.group('Task create payload builder', () => {
  test('maps normalized DTO fields into the task persistence payload', ({ assert }) => {
    ...
    assert.deepEqual(payload.domain_tags, [])
    assert.deepEqual(payload.project_business_domains, [])
    assert.equal(payload.due_date.toISO(), resolvedDueDate.toISO())
  })

  test('captures the Project business-domain context and never retains Task-level domain values', ({ assert }) => {
    const payload = buildCreateTaskPersistencePayload(
      makeCreateTaskDTO(),
      VALID_UUID,
      { id: VALID_UUID_3, category: 'todo' },
      DateTime.fromISO('2026-04-22T00:00:00.000Z'),
      ['saas', 'security']
    )

    assert.deepEqual(payload.project_business_domains, ['saas', 'security'])
  ...
  test('fills nullable persistence fields consistently for optional DTO values', ({ assert }) => {
    ...
    assert.isNull(payload.project_sprint_id)
