...
function makeHarness(activeAssigneeId: string | null = null): AssignmentHarness {
  ...
  return {
    ...
    skillReader: new (class extends TaskSkillReader {
      ...
      listProjectTaskSkills = () => Promise.resolve([])
      listActiveProficiencyLevels = () => Promise.resolve([])
      findActiveSkillIds = () => Promise.resolve([])
      findSkillSummariesByIds = () => Promise.resolve([])
      resolveSkillIdsByCategoryCodes = () => Promise.resolve([])
      findTaskRequirementReferenceFacts = () =>
        Promise.resolve({ skills: [], proficiencyLevels: [] })
      findProficiencyLevelsByIds = () => Promise.resolve([])
      findProficiencyLevelById = () => Promise.resolve(null)
      findRubricVersion = () => Promise.resolve(null)
      findProjectRole = () => Promise.resolve(null)
    })(),
    transaction,
  }
}
...
test.group('Synchronize task assignment', () => {
  ...
  test('refuses to assign a user who is below a mandatory task skill level', async ({ assert }) => {
    const harness = makeHarness()
    harness.skillReader.getTaskSkillEligibility = () =>
      Promise.resolve({
      isEligible: false,
      unmetRequirements: [
        {
