...
import { TaskOrgRole, TaskProjectRole } from '../task-authoring/role_contracts.js'
import type {
  ...
  UpdateFieldsResult,
} from '../task-authoring/task_types.js'
...
export function canUpdateTaskStatus(ctx: TaskPermissionContext): PolicyResult {
  // Organization owners/admins govern every project in their organization.
  // Project membership narrows member access; it must not constrain org governance.
  if (isOrgOwnerOrAdmin(ctx.actorOrgRole)) return PR.allow()
 ...
 *
 * Organization owners/admins govern every project in their organization and
 * therefore have the same full task-edit capability as a project owner.
 *
...
export function canUpdateTaskFields(
  ...
  // Org owner/admin — no field restrictions inside their organization.
...
/**
 * Marketplace detail is a read-only opportunity preview, not project-board
 * access. Internal opportunities are visible only to approved members of the
 * owning organization; public opportunities additionally require a public
 * project that allows external contributors.
 */
export function canViewTaskOnMarketplace(
  ctx: TaskPermissionContext & {
    isApprovedOrganizationMember: boolean
    projectVisibility?: 'public' | 'private' | 'team' | null
    allowExternalContributors?: boolean | null
  }
): PolicyResult {
  if (isSameId(ctx.taskCreatorId, ctx.actorId)) return PR.allow()
  if (ctx.taskAssignedTo !== null && isSameId(ctx.taskAssignedTo, ctx.actorId)) return PR.allow()
  ...
  if (ctx.taskVisibility === 'internal') {
    return ctx.isApprovedOrganizationMember
      ? PR.allow()
      : PR.deny('Bạn không thuộc tổ chức của opportunity này')
  }

  if (
    PUBLIC_TASK_VISIBILITIES.has(ctx.taskVisibility ?? '') &&
    ctx.projectVisibility === 'public' &&
    ctx.allowExternalContributors === true
  ) {
    return PR.allow()
  }

  return PR.deny('Opportunity này không được mở trong Marketplace')
}
...
export function resolveTaskCollectionReadScope(
  ...
  if (ctx.projectId && ctx.actorProjectRole) {
...
export function calculateTaskPermissions(ctx: TaskPermissionContext): {
  ...
  // Discussion follows the same boundary as task visibility. Anyone who can
  // open the task (including a read-only project viewer) can add context to it.
  const canComment = canViewTask(ctx).allowed

  return { isCreator, isAssignee, canEdit, canDelete, canAssign, canChangeStatus, canComment }
}
