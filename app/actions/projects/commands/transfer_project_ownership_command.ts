...
export default class TransferProjectOwnershipCommand {
    ...
    private async isOrgAdminOrOwner(userId: number, organizationId: number, trx: any): Promise<boolean> {
    ...
    private async sendNotifications(project: Project, oldOwnerId: number, newOwnerId: number): Promise<void> {
        try {
