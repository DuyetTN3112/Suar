...
const realtimeSessionListenerDependencies: NotificationRealtimeSessionListenerDependencies = {
  revoke: (recipientId, options) => notificationRealtimeSessionRevoker.revoke(recipientId, options),
}

emitter.on('user:logout', (event) =>
  handleNotificationRealtimeUserLogout(event, realtimeSessionListenerDependencies)
)
emitter.on('user:deactivated', (event) =>
  handleNotificationRealtimeUserDeactivated(event, realtimeSessionListenerDependencies)
)
