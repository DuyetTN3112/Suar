import CreateReviewObservationCommand from '#modules/reviews/actions/commands/observation/create_review_observation_command'
...
import GetUserReviewsQuery from '#modules/reviews/actions/queries/review-core/get_user_reviews_query'
import ListUserReviewHistoryQuery from '#modules/reviews/actions/queries/review-core/list_user_review_history_query'
import GetReviewEvidencesQuery from '#modules/reviews/actions/queries/review-submission/get_review_evidences_query'
...
export interface ReviewTaskActionFactoryDependencies {
  ...
  observationContexts?: ReviewObservationAuthoringContextReader
  observationWriter?: ReviewObservationWriter
}
...
export class ReviewTaskActionFactory {
  ...
  makeFinalizeTaskReviewWorkflowCommand(
    execCtx: ReviewActionContext
  ): FinalizeTaskReviewWorkflowCommand {
    return new FinalizeTaskReviewWorkflowCommand(
      execCtx,
      this.dependencies.externalDependencies.actorAccess,
      this.requireTaskWorkflows()
    )
  }
  ...
  makeOpenTaskReviewDisputeCommand(execCtx: ReviewActionContext): OpenTaskReviewDisputeCommand {
    return new OpenTaskReviewDisputeCommand(execCtx, this.requireTaskWorkflows())
  }
  ...
  makeCreateReviewObservationCommand(
    ...
    return new CreateReviewObservationCommand(execCtx, {
      contexts: this.requireObservationContexts(),
      writer: this.requireObservationWriter(),
    })
  }
  ...
  private requireObservationContexts(): ReviewObservationAuthoringContextReader {
    if (!this.dependencies.observationContexts) {
      throw new Error('Review observation authoring context capability is not configured')
    }
    return this.dependencies.observationContexts
  }

  private requireObservationWriter(): ReviewObservationWriter {
    if (!this.dependencies.observationWriter) {
      throw new Error('Review observation writer capability is not configured')
    }
    return this.dependencies.observationWriter
  }
}
