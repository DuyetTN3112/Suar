...
function hasTooManyRepeats(str: string, maxRepeat = 20): boolean {
...
function sanitizeMessage(str: string): string {
  ...
  str = str.replace(
    /(.)([\u0300-\u036F\u1AB0-\u1AFF\u1DC0-\u1DFF\u20D0-\u20FF\uFE20-\uFE2F]{1,})/gu,
    (match, base, combining) => base + combining.substring(0, 1)
  )
  // Giới hạn độ dài của chuỗi lặp lại
  str = str.replace(/(.)(\1{10,})/g, (match, char, repeats) => char.repeat(10))
