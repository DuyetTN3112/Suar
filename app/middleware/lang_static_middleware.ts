...
export default class LangStaticMiddleware {
  ...
  async handle(ctx: HttpContext, next: () => Promise<void>) {
    ...
    // Debug
    console.log(`[LangStaticMiddleware] Processing URL: ${url}`)
    ...
    if (url.startsWith('/lang/')) {
      try {
        ...
        // Log để debug
        console.log(`[LangStaticMiddleware] URL parts:`, parts)
        ...
        console.log(`[LangStaticMiddleware] Looking for file at: ${langPath}`)
        // Kiểm tra tệp có tồn tại không
        if (!existsSync(langPath)) {
          console.log(`[LangStaticMiddleware] File not found: ${langPath}`)
          return response.status(404).send('Not Found')
        }
        console.log(`[LangStaticMiddleware] Serving file: ${langPath}`)
