...
const PREVIOUS_OWNER_COMMENT =
  'managed-by:20260726012000_expand_domain_event_outbox_review_confirmed'
const OWNER_COMMENT =
    ...
    await this.db.rawQuery(`
      ...
      BEGIN
        IF to_regclass('public.domain_event_outbox') IS NULL THEN
          RAISE EXCEPTION
            'domain_event_outbox must exist before enabling dispute resolved events';
        END IF;
    ...
    await this.db.rawQuery(`
      ...
      BEGIN
        ...
        IF EXISTS (
          ...
          RAISE EXCEPTION
            'refusing to disable dispute resolved outbox support while durable rows remain';
        END IF;
