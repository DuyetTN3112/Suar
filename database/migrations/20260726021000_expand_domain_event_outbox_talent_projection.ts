    ...
    await this.db.rawQuery(`
      ...
      BEGIN
        IF to_regclass('public.domain_event_outbox') IS NULL THEN
          RAISE EXCEPTION 'domain_event_outbox must exist before enabling talent projection events';
        END IF;
    ...
    await this.db.rawQuery(`
      ...
      BEGIN
        ...
        IF EXISTS (
          ...
          RAISE EXCEPTION 'refusing to disable talent projection outbox support while durable rows remain';
        END IF;
        SELECT pg_get_constraintdef(oid), obj_description(oid, 'pg_constraint')
          INTO constraint_definition, constraint_owner
        FROM pg_constraint
