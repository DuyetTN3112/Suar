...
const OWNER_COMMENT =
  'managed-by:20260726018000_expand_domain_event_outbox_review_submitted'
    ...
    await this.db.rawQuery(`
      ...
      BEGIN
        IF to_regclass('public.domain_event_outbox') IS NULL THEN
          RAISE EXCEPTION 'domain_event_outbox must exist before enabling review submitted events';
        END IF;
  ...
  override async down() {
    ...
    await this.db.rawQuery(`
      ...
      BEGIN
        ...
        IF EXISTS (
          SELECT 1 FROM public.domain_event_outbox WHERE event_name = 'review:submitted'
        ) THEN
          RAISE EXCEPTION 'refusing to disable review submitted outbox support while durable rows remain';
        END IF;
        SELECT pg_get_constraintdef(oid), obj_description(oid, 'pg_constraint')
          INTO constraint_definition, constraint_owner
        FROM pg_constraint
