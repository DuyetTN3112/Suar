...
const PREVIOUS_CONSTRAINT_OWNER =
  'managed-by:20260726026000_add_durable_auth_session_events'
    ...
    await this.db.rawQuery(`
      ...
      BEGIN
        IF EXISTS (
          ...
          WHERE event_name = 'project:lifecycle:changed:v1'
        ) THEN
          RAISE EXCEPTION 'refusing to disable project lifecycle events while durable rows remain';
        END IF;
        SELECT pg_get_constraintdef(oid), obj_description(oid, 'pg_constraint')
          INTO constraint_definition, constraint_owner
        FROM pg_constraint
